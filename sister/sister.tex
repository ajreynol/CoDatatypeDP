...
\begin{abstract}
Datatypes and codatatypes are useful to represent infinite and potentially
infinite data structures and processes. We describe a decision procedure that
combines reasoning about datatypes and codatatypes. The procedure has been
integrated in CVC4, a state-of-the-art SMT (satisfiability modulo theories)
solver, which can be used both as a general constraint solver and as an
automatic theorem prover. An evaluation based on problems generated from
interactive formal proofs developed using the Isabelle proof assistant
demonstrates the potential of our decision procedure.
\end{abstract}
...
* much focus on FOL
* two main approaches:
  superposition---e.g., E, SPASS, Vampire
  SMT (satisfiability modulo theory) solvers---e.g., CVC4, veriT, Yices, Z3
* SMT is also useful for model finding, or constraint solving: given some   
  constraints, find an assignment of its variables that satisfy them

* strength of the SMT approach is that it can compose different efficient
  decision procedures and other solvers for various theories---e.g., equality,
  linear arithmetic, and first-order quantifiers.
* the main scientific question is: which theories would be useful to
  applications, and which ones can be supported efficiently in SMT solvers?
...
calculus and is composable via the Nelson--Oppen \shortcite{nelson-oppen-1979}
method.
...
problems generated from Isabelle\slash HOL
formalizations demonstrates the usefulness of the approach (Section~\ref{sec:evaluation-on-isabelle-problems}).
...
We fix a signature $\Sig = (\Types, \Funcs)$ consisting of a set of types $\Types$ and a
...
inspired by Barrett et al.\ \shortcite{barrett-et-al-2007} but at
...
datatypes in CVC4.
We want to thank everyone who helped us with the longer versions of
this paper.
