...
%   pdfauthor={Andrew Reynolds, Jasmin Christian Blanchette},
%   pdfkeywords={Decision Procedures, Automated Theorem Proving, Satisfiability Modulo Theories},
%   pdfborder={0 0 0},
%   draft=false,
%   bookmarksnumbered,
%   bookmarks,
...
\pdfinfo{
  ...
  /Keywords (Decision Procedures, Automated Theorem Proving, Satisfiability Modulo Theories)
}
...
have emerged as useful tools in a number of applications~\cite{DeMoura2011SMT}.
These solvers are one of the most powerful ways to prove theorems in classical
first-order logic, and are also useful for constraint solving, or model finding:
...
%axiom.
%
Third, \emph{it must be possible to express cyclic values as closed terms.} %This is necessary when generating models.
...
Conference on Automated Deduction (CADE-25) in
...
\emph{J.\ Autom.\ Reasoning}. % \cite{reynolds-blanchette-201x-codata3}.
We refer to these for a description of related work.
...
and disequalities ($\not\teq$) between first-order terms, %$\Sig$-terms,
...
trivial entries $\ec{t} \mapsto {\Varec{t}}$.
%
\begin{enumerate}
...
\\
  ...
  {\mathop{\big|\kern-.25ex\big|}\vthinspace}_{i=1}^n\, \DISC{i} \in \Ec %\ROR
 % \ROR \cdots \ROR \DISC{m} \in \Ec
}
\)
\rn{Split}
\\[6\jot]
...
%$\Ec$ is %$\thD$-
%satisfiable.
We proved the following properties of derivation trees generated by the calculus
\cite{reynolds-blanchette-2015-codata}.
...
The proof of solution soundness %is constructive in that it
provides a method for constructing a model for a saturated configuration.
