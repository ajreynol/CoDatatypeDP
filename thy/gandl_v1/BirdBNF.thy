theory BirdBNF imports SternBrocotBNF begin

definition bird :: "fraction tree"
where "bird = tree_recurse (inverse' o succ) (succ o inverse') (1, 1)"

lemma bird_unfold:
  "bird = Node (1, 1) (tree_inverse \<diamond> (tree_succ \<diamond> bird)) (tree_succ \<diamond> (tree_inverse \<diamond> bird))"
by(auto simp add: bird_def tree_recurse_def map_tree_tree_ap_tree_pure intro: tree.expand)

lemma bird_simps:
  "root bird = (1, 1)"
  "left bird = tree_inverse \<diamond> (tree_succ \<diamond> bird)"
  "right bird = tree_succ \<diamond> (tree_inverse \<diamond> bird)"
...
lemma mirror_pure: "mirror (tree_pure x) = tree_pure x"
by(coinduction rule: tree.coinduct) simp

lemma mirror_tree_ap: "mirror (f \<diamond> x) = mirror f \<diamond> mirror x"
by(coinduction arbitrary: f x) auto

lemma inverse'_inverse': "inverse' \<circ> inverse' = id"
by(simp add: fun_eq_iff)

lemma "mirror bird = tree_inverse \<diamond> bird" (is "?lhs = ?rhs")
proof -
  let ?R = "\<lambda>t. Node (1, 1) (tree_succ \<diamond> (tree_inverse \<diamond> t)) (tree_inverse \<diamond> (tree_succ \<diamond> t))"
  have "mirror bird = ?R (mirror bird)"
    by(rule tree.expand)(simp add: bird_simps mirror_tree_ap mirror_pure)
  note tree_recurse_unique[OF this[unfolded map_tree_tree_ap_tree_pure tree.map_comp]]
  moreover
  def t \<equiv> "tree_inverse \<diamond> bird"
  have "t = ?R t"
    by(rule tree.expand)(simp add: t_def bird_simps tree_ap_composition[symmetric] o_assoc inverse'_inverse')
  note tree_recurse_unique[OF this[unfolded map_tree_tree_ap_tree_pure tree.map_comp]]
...
lemma even_odd_mirror_pure: fixes even shows
  "even_odd_mirror even (tree_pure x) = tree_pure x"
by(coinduction arbitrary: even) auto

lemma even_mirror_pure: "even_mirror (tree_pure x) = tree_pure x"
  and odd_mirror_pure: "odd_mirror (tree_pure x) = tree_pure x"
by(simp_all add: even_mirror_def odd_mirror_def even_odd_mirror_pure)

lemma even_odd_mirror_tree_ap: fixes even shows
...
lemma even_mirror_tree_ap: "even_mirror (f \<diamond> x) = even_mirror f \<diamond> even_mirror x"
  and odd_mirror_tree_ap: "odd_mirror (f \<diamond> x) = odd_mirror f \<diamond> odd_mirror x"
by(simp_all add: even_mirror_def odd_mirror_def even_odd_mirror_tree_ap)
...
lemmas even_odd_mirror_simps =
  ...
  even_mirror_tree_ap odd_mirror_tree_ap
...
definition intermediate where
  "intermediate t = Node (1, 1) (Node (1, 2) (rssr t) (rsrs t)) (Node (2, 1) (srsr t) (srrs t))"

lemma min_Suc: "min n (Suc n) = n"
by(simp add: min_def)
...
proof -
  ...
  have "odd_mirror bird = ?R (odd_mirror bird)"
    by(auto 4 3 intro: tree.expand simp add: even_mirror_simps odd_mirror_simps bird_simps even_odd_mirror_simps tree_ap_composition[symmetric] o_assoc map_tree_tree_ap_tree_pure tree.map_comp)
  note bird_rec_unique[OF this]
  moreover
  have "stern_brocot = ?R stern_brocot"
    by(rule tree.expand)(auto intro: tree.expand simp add: stern_brocot_simps tree_ap_composition[symmetric] o_assoc map_tree_tree_ap_tree_pure tree.map_comp rssrsr_rssr srrs_ss)
  note bird_rec_unique[OF this]
...
theorem bird_rationals:
  assumes "m > 0" "n > 0"
  shows "root (traverse_tree (odd_mirror_path (mkPath m n)) (tree_toRat \<diamond> bird)) = Fract (int m) (int n)"
...
theorem bird_rationals_not_repeated:
  "root (traverse_tree path (tree_toRat \<diamond> bird)) = root (traverse_tree path' (tree_toRat \<diamond> bird))
