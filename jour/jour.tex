...
\centering
\begin{enumerate}
\item Let $\mathcal{U}$ be a partition of the codatatype equivalence classes of $\Ec$ such that $\ec{t} =_{\mathcal{U}} \ec{u}$ for distinct $\ec{t}, \ec{u}$ if and only if:
\begin{itemize}
\item $\const{C}( t_1, \ldots, t_k ) \in \ec{t}$ and $\const{C}( u_1, \ldots, u_k ) \in \ec{u}$.
\end{itemize}
