...
\newcommand\xend{{\hfill\llap{$\scriptstyle\blacksquare$}}}
...
%    and the SMT-LIB standard, and codatatypes as their duals
...
\cite{kozen-1983,endrullis-et-al-2011}.
% [etc.]
...
as long as the argument types coincide. % \cite[Section~3]{blanchette-et-al-2014-impl}.
...
\[\begin{aligned}[t]
      ...
      \tau,\: \const{tl}{:}\; \ty{list}_{\vthinspace\tau}) \\%[-.5\jot]
...
given
\[\keyw{datatype}\;\, \ty{x} \,=\, \const{C}(\const{s}{:}\; \ty{int}) \mid \const{D} \mid \const{E}\]
...
%The last premise is admittedly rather technical.
...
%This premise is not strictly necessary in practice,
...
\end{array}\]
%
Here, $\ec{u}$ denotes the equivalence class $\{ u, \const{C}( z ) \},$
$\ec{v}$ denotes $\{ v, \const{C}( z ), s \},$ and so on.
...
Thus, we have
$\expand{\Val \ec{y}}{ t } = \expand{\Val \ec{x}}{ \Val \ec{x} } = \Val \ec{x},$ and %thus
(2) holds for $\Val.$
...
%For instance, up to renaming,
%$\interp{\J}{u}$ is $\const{C}( \MU {\Varec{z}}.\; \const{E}( {\Varec{z}} ) ),$
%$\interp{\J}{v}$ and $\interp{\J}{s}$ are $\const{D}( \MU {\Varec{z}}.\; \const{E}( {\Varec{z}} ) ),$ and so on.
\xend
\end{examplex}
...
\interp{\J}{s} \,=\, \interp{\J}{v} & = & \const{D}( \MU {\Varec{z}}.\; \const{E}( {\Varec{z}} ) ) &
\interp{\J}{x} & {}\,=\,{} & \const{C}( \const{D}( \MU {\Varec{z}}.\; \const{E}( {\Varec{z}} ) ) ) &
\interp{\J}{z} & = & \MU {\Varec{z}}.\; \const{E}( {\Varec{z}} )
\end{array}\]
%It can be shown that
It is easy to see that $\J$ satisfies the constraints

\vskip\abovedisplayskip

\noindent
\hbox{}{\color{white}\xend}$u \teq \const{C}( z )$\hfill
$v \teq \const{D}( z )$\hfill
$w \teq \const{E}( y )$\hfill
$x \teq \const{C}( v )$\hfill
$v \teq\nobreak s$\hfill
$z \not\teq v$\xend
\end{examplex}
...
For \rn{Split}, we add the exhaustiveness clause %of the form
...
\item Repeat until $U \neq \emptyset$:
\begin{itemize}
\item[2.1.] For some $\ec{t} \in U,$ $\rn{Traverse}( \ec{t}, U, \emptyset ).$
\end{itemize}
\end{enumerate}
...
\end{enumerate}
\vspace*{-\topsep}
\caption{\,Algorithm for applying the \rn{Acyclic} rule}
...
\end{enumerate}
\vspace*{-\topsep}
\caption{\,Algorithm for applying the \rn{Unique} rule}
...
%To recognize when the premise of \rn{Unique} holds,
...
%Concretely,
For codatatypes, we enumerate a stream of \relax{every} $\MU$-term of our signature $\Sig$ in a fair manner,
...
For example, the codatatype $\ty{stream}_{\vthinspace\ty{u}}$ %, where $\ty{u}$ is uninterpreted.
...
online.%
\footnote{\url{http://lara.epfl.ch/~reynolds/JAR-CADE2015-cdt/} or
\url{http://www21.in.tum.de/~blanchet/JAR-CADE2015-cdt/}}
