...
allowed, even though it has no base case; the
...
Berlin, Germany. The article extends the conference paper with additional
background material on (co)datatypes (Section~\ref{sec:co-datatypes}), more
elaborated metatheoretical proofs (Section~\ref{sec:the-decision-procedure}),
...
The operator $\bigwedge_{\,i}\, \varphi_i$ abbreviates a conjunction
...
We fix a signature $\Sig = (\Types, \Funcs)$. The types are partitioned as
...
$v_1, w_1 : \delta_1,\,\ldots,\,v_\ell, w_\ell : \delta_\ell$, based on
coinduction witnesses $R_1,\ldots,R_\ell$ that must be supplied upon use:
%
\[
\begin{aligned}[t]
\text{Coinduction:}\quad
\AXC{\strut$\begin{gathered}\textstyle \smash{\bigwedge_{\,i}\, R_i[v_i, w_i]} \\[-\jot]\textstyle
\bigwedge_{\,i}\, \forall v\; w.\;\, R_i[v, w] \medrightarrow
  \bigwedge_{\,j}\, \const{d}_{\negvthinspace j}(v) \mathrel\land \const{d}_{\negvthinspace j}(w)
  \medrightarrow \bigwedge_{\,k}\, \const{s}_{i\negvthinspace j}^{\,k}(v) \sim \const{s}_{i\negvthinspace j}^{\,k}(w)
\end{gathered}$}
...
\vthinspace$\keyw{codatatype}~\ty{a} = \const{A}(\ty{a})$, which has a cardinality
of one; its unique value is $\MU a.\; \const{A}(a)$. Other specimens are
$\ty{stream}_{\,\ty{unit}\,}$
...
In this section, we describe the derivation rules of the calculus for $\thD$.
...
Moreover, at the end of this phase, each equivalence class $\ec{t}$ contains (at most) 
...
it is enough for our purposes to select an arbitrary constructor term $\const{C}( \vec t\, ) \in \ec{t}$ among these.
...
%AJR : this sentence is necessary, since we are abusing the \Varec notation a bit
For a term $t \in \tEc$, 
we write $\Varec{t}$ to denote the variable associated with equivalence class $\ec{t}$.
...
%%% disagree.
\subsection{Theory Solver for $\thD$}
...
\item Repeat until $U \neq \emptyset$.
\begin{itemize}
\item[\ ] For some $\ec{t} \in U$, $\mathrm{traverse}( \ec{t}, U, \emptyset )$
\end{itemize}
\end{enumerate}

\flushleft $\mathrm{traverse}( \ec{t}, U, P )$:
\begin{enumerate}
\item If $\ec{t} \in P$, apply \rn{Acyclic} to $t$.
\item If $\ec{t} \in U$,
\begin{enumerate}
\item If $\const{C}( t_1, \ldots, t_k ) \in \ec{t}$, then for each $j = 1, \ldots k$,
\begin{itemize}
\item[\ ] $\mathrm{traverse}( \ec{t_j}, U, P \cup \{ \ec{t} \} )$
\end{itemize}
\item $U := U \textbackslash \{ \ec{t} \}$.
\end{enumerate}
\end{enumerate}
\vspace*{-2pt} %% TYPESETTING HACK
...
\centering
\begin{enumerate}
\item Let $\mathcal{U}$ be a partition of the equivalence classes of $\Ec$ such that $\ec{t} =_{\mathcal{U}} \ec{u}$ for distinct $\ec{t}, \ec{u}$ if and only if:
\begin{itemize}
\item $t\typ{\delta}$, $u\typ{\delta}$ where $\delta \in \Codata$, and $\const{C}( t_1, \ldots, t_k ) \in \ec{t}$ and $\const{C}( u_1, \ldots, u_k ) \in \ec{u}$.
\end{itemize}
\item Repeat until $\mathcal{U}$ is unchanged
\begin{enumerate}
\item $\mathcal{U}' := \emptyset$
\item for each $U_i \in \mathcal{U}$:
\begin{itemize}
\item[\ ] Let $\mathcal{U}_i$ be a partition of $U_i$ such that $\ec{t} =_{\mathcal{U}_i} \ec{u}$ for distinct $\ec{t}, \ec{u}$ if and only if:
\begin{itemize}
\item $\const{C}( t_1, \ldots, t_k ) \in \ec{t}$ and $\const{C}( u_1, \ldots, u_k ) \in \ec{u}$, and
\item $\ec{t_j} =_{\mathcal{U}} \ec{u_j}$ for each $j = 1, \ldots, k$.
\end{itemize}
\item[\ ] $\mathcal{U}' := \mathcal{U}' \cup \mathcal{U}_i$.
\end{itemize}
\item $\mathcal{U} := \mathcal{U}'$
\end{enumerate}
\item If distinct $\ec{t}, \ec{u} \in U$ for some $U \in \mathcal{U}$, apply \rn{Unique} to $t,u$.
\end{enumerate}
\vspace*{-2pt} %% TYPESETTING HACK
...
Instead, we use the algorithms in Figures~\ref{fig:acyclic-algo} and~\ref{fig:uniqueness-algo}
for determining whether the rules \rn{Acyclic} and \rn{Unique} apply to our set $\Ec$.

For \rn{Acyclic}, Figure~\ref{fig:acyclic-algo} considers the set $U$ of all datatype equivalence classes of $\Ec$.
We choose an arbitrary $\ec{t}$ in $U$ and call the recursive subprocedure $\mathrm{traverse}$,
...
If a call to $\mathrm{traverse}( \ec{t}, U, P )$ is such that $\ec{t} \in P$,
...
then we recursively call $\mathrm{traverse}$ on each of $\ec{t_1}, \ldots, \ec{t_n}$.
If this succeeds, we remove $\ec{t}$ from $U$.

For \rn{Unique}, Figure~\ref{fig:uniqueness-algo} considers a partition $\mathcal{U}$ of the equivalence classes of our set $\Ec$
...
We write $\ec{t} =_{\mathcal{U}} \ec{u}$ to denote that $\ec{t}$ and $\ec{u}$ reside in the same subset within $\mathcal{U}$.
We then refine $\mathcal{U}$ by 
constructing a partition $\mathcal{U}_i$ of each $U_i \in \mathcal{U}$ such that distinct $\ec{t}$ and $\ec{u}$ reside in the same subset of $\mathcal{U}_i$ 
if and only if they contain constructor terms $\const{C}( t_1, \ldots, t_k )$ and $\const{C}( u_1, \ldots, u_k )$
where each $t_j, u_j$ are such that $\ec{t_j}$ and $\ec{u_j}$ reside in the same subset of $\mathcal{U}$ for each $j=1, \ldots, k$.
Let $\mathcal{U'}$ be the union of each of these partitions.
We update $\mathcal{U}$ to $\mathcal{U'}$.
This refinement is repeated until $\mathcal{U}$ is left unchanged.
If the resulting partition $\mathcal{U}$ contains a subset $U$ having two distinct equivalence classes $\ec{t}$ and $\ec{u}$, then \rn{Unique} applies to $t$ and $u$.
This algorithm is similar to the classic algorithm for minimizing deterministic finite automata (DFA). \textbf{maybe cite?}
...
When instructed, the implementation produces models for satisfiable inputs.
As described in Section~\ref{sec:termination-and-correctness},
given a saturated set $\Ec$, we may construct a map $\ValC$ from 
the equivalence classes of $\Ec$ to closed normal $\MU$-terms.
Recall our construction of $\ValC$ requires picking values $\MU {\Varec{x}}.\; t$ for each $\Varec{x} \in \FV( \ValC )$
such that $\MU {\Varec{x}}.\; t$ does not occur in the set $\BAD_{\smash{\J}}^{\Varec{x}}( \ValC )$.
To pick values $\MU {\Varec{x}}.\; t$, we use a fair enumerator for each (co)datatype $\tau$ which enumerates the values
of type $\tau$ in a normal interpretation, which are closed normal $\MU$-terms.
Concretely, we enumerate a stream of \emph{every} $\MU$-term of our signature $\Sigma$ in a fair manner,
discarding whose that have free variables or are not normal.
For those that are closed and normal, we then check whether they occur in the set $\BAD_{\smash{\J}}^{\Varec{x}}( \ValC )$,
which we do not explicitly construct.
%We do not explicitly construct the set $\BAD_{\smash{\J}}^{\Varec{x}}( \ValC )$.
Instead, to check if a term $\MU {\Varec{x}}.\; t$ occurs in the set $\BAD_{\smash{\J}}^{\Varec{x}}( \ValC )$, 
...
This can be efficiently checked by matching the term $\MU {\Varec{x}}.\; t$ with the term $\MU {\Varec{y}}.\; u$.
...
soundness, without which the generated models would often be spurious. The
contributions to proving are less clear; they depend on how often the need for
acyclicity or uniqueness arises in practice.

To evaluate the decision procedure for proving conjectures,
...
added unpublished formalizations about Bird and Stern--Brocot trees
by Peter Gammie and Andreas Lochbihler (\gandl,
265~goals). To exercise the datatype support, formalizations about lists and trees
...
development version of CVC4 (\textbf{TODO: which revision?}) %AJR : version is from September 15 2015, git doesn't give nice revision numbers unfortunately
and to
Z3 4.3.2 %(\textbf{TODO: really?}) AJR : yes
for comparison, each running for
...
subsume the weaker ones in practice. We encountered only one
(\textbf{TODO: FIXME}) goal (out of $2 \times 4458$)
...
%
Four proofs were found thanks to
the acyclicity rule and 17~required uniqueness. Among the latter,
some were simple proofs of the form
\keyw{by}~\textit{coinduction}~\textit{auto}
in Isabelle \cite{blanchette-et-al-2014-impl}, while others
required more elaborate arguments---for example:
%
\begin{quote}
...
Finally, it might be worthwhile to
