...
\textbf{Mention this coincides with DFA minimization algorithm.}
...
The implementation is able to produce models for satisfiable inputs.
As described in Section~\ref{sec:termination-and-correctness},
given a saturated set $\Ec$, we may construct a map $\ValC$ from variables (corresponding to the equivalence classes of $\Ec$) to closed normal $\MU$-terms.
%In practice, this requires a fair enumerator for each (co)datatype $\tau$.

%Recall that to assign a value $\MU {\Varec{x}}.\; t$ to variable 

%We do not explicitly construct the set $\BAD_{\smash{\J}}^x( \Val )$.
%Instead, 
