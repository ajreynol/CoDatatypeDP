...
\centering
\begin{enumerate}
\item Let $\mathcal{U}$ be a partition of the equivalence classes of $\Ec$ such that $\ec{x} =_{\mathcal{U}} \ec{y}$ if and only if:
\begin{itemize}
\item $\const{C}( x_1, \ldots, x_k ) \in \ec{x}$ and $\const{C}( y_1, \ldots, y_k ) \in \ec{y}$.
\end{itemize}
...
\begin{itemize}
\item[\ ] Let $\mathcal{U}_i$ be a partition of $U_i$ such that $\ec{x} =_{\mathcal{U}_i} \ec{y}$ for distinct $\ec{x}, \ec{y}$ if and only if:
\begin{itemize}
\item $\const{C}( x_1, \ldots, x_k ) \in \ec{x}$ and $\const{C}( y_1, \ldots, y_k ) \in \ec{y}$, and
\item $\ec{x_j} =_{\mathcal{U}} \ec{y_j}$ for each $j = 1, \ldots, k$.
\end{itemize}
\item[\ ] $\mathcal{U}' := \mathcal{U}' \cup \mathcal{U}_i$.
\end{itemize}
\item $\mathcal{U} := \mathcal{U}'$
\end{enumerate}
\item If distinct $t\typ{\delta}, s\typ{\delta} \in U \in \mathcal{U}$ where $\delta \in \Codata$, apply \rn{Unique} to $t,s$.
\end{enumerate}
\vspace*{-2pt} %% TYPESETTING HACK
