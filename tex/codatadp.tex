...
whose root nodes are the result of applying the rule \rn{Split} on $t : \delta$,
...
First, we need to define the set of interpretations of the theory $\thD$,
...
each pair of constructor terms $\const{C}_{\negvthinspace j}( \vec t )$ and $\const{C}_{\vthinspace\jPrime}( \vec u )$ in $\ec{z}$
...
%$\Val_1 \sigma \ec{y}$ contains a strict bisimilar subterm 
...
%where $\Val^\AST \ec{t}\interp{w_i} = \Val^\AST \ec{u_i}$ for $i = 1, \ldots, n$.
...
For each $i = 1, \ldots, n$, let $u_\iPrime$ be the $i$\vvthinspace th argument of $t_u$.
By Lemma~\ref{lem:mu-norm-arg}, $t_u\interp{u_\iPrime} \vsim \Val^\AST \ec{u_i}$.
Thus $t_u\interp{u_\iPrime} \vsim \Val^\AST \ec{t}\interp{ w_i }$. % for each $i = 1, \ldots, n$.
...
$\ec{u}$ must contain a term of the form $\const C_{\vthinspace\jPrime}\bigl( \const s^1_{\vthinspace\jPrime}( u ), \ldots, \const s^{n}_{\vthinspace\jPrime}( u ) \bigr)$ for some $\jPrime$.
%By Lemma~\ref{lem:model-completion}.2 and due to our construction of $\Val^\AST$, we have $\Val^\AST \ec{u}$ is of the form
%$\MU {\Varec{u}}.\; \const C_{\vthinspace\jPrime}( w_1, \ldots, w_n )$,
%where $\Val^\AST \ec{u} \interp{ w_i } = \Val^\AST \ec{ \const s^i_{\vthinspace\jPrime}( u )}$ for $i = 1, \ldots, n$. 
Since \rn{Inject} and \rn{Clash} do not apply to $\Fc$, 
by our construction of $\Val^\AST$ we have $\Val^\AST \ec{u}$ is of the form
$\MU {\Varec{u}}.\; \const C_{\vthinspace\jPrime}( w_1, \ldots, w_n )$,
where for each $i = 1, \ldots, n$,
$\muvar(w_i) = {\Varec{\const s^i_{\vthinspace\jPrime}( u )}}$ and thus by Lemma~\ref{lem:model-completion}.2, 
$\Val^\AST \ec{u} \interp{ w_i } = \Val^\AST \ec{ \const s^i_{\vthinspace\jPrime}( u )}$. 
If $j = \jPrime$, then $\M( t )$ is $\alpha$-equivalent with $\Val^\AST \ec{u} \interp{w_k}$, which is equal to $\Val^\AST \ec{ \const s^k_{\negvthinspace j}( u )}$,
which is $\Val^\AST \ec{t}$.
If $j \neq \jPrime$, we are free to interpret $\const s^k_{\negvthinspace j}( \M( u ) )$ as the value in $\M( \tau )$ that is $\alpha$-equivalent with $\Val^\AST \ec{ t }$.
...
replace $\const{C}_{\negvthinspace j}( \vec t ) \teq \const{C}_{\vthinspace\jPrime}( \vec u )$ with $\bot$ when $j \neq \jPrime$,
replace all selector terms of the form $\const{s}^{\,k}_{\negvthinspace j}( \const{C}_{\negvthinspace j}(t_1,\ldots,t_n) )$ by $t_k$,
and replace all occurrences of discriminators $\const{d}_{\negvthinspace j}( \const{C}_{\vthinspace\jPrime}( \vec t ) )$ by $\top$ if $j = \jPrime$ and $\bot$ otherwise.
