...
Codatatypes are freely generated by their constructors, but unlike for datatypes
infinite constructor terms are also legitimate values
...
A cyclic constraint such as
$\const{x} \teq \const{S}(\const{x})$ %, where $\const{C}$ is a constructor,
is unsatisfiable for a datatype but satisfiable for a codatatype.
...
to enumerate them.} For finite model finding, requires a $\mu$ notation,
e.g., $\const{SCons}(3,\, \mu s.\; \const{SCons}(1,\, \const{SCons}(4,\, s)))$
represents the lasso-shaped sequence $3, 1, 4, 1, 4, \ldots$\kern0.1em. %%% TYPESETTING: kern
\end{itemize}
