...
Isabelle theories demonstrates the potential of the procedure for both proving
and model finding.
\end{abstract}
...
as a calculus (Section~\ref{sec:the-calculus}) and is designed to be composable
via the Nelson--Oppen method \cite{nelson-oppen-1979}.
...
to consider them together. There are, however, a number of important
differences:
%
\begin{itemize}
\item
...
the term $\const{SCons}(3,\, \mu s.\; \const{SCons}(1,\, \const{SCons}(4,\, s)))$
denotes the lasso-shaped sequence $3, 1, 4, 1, 4, \ldots$\kern0.1em. %%% TYPESETTING: kern
\end{itemize}
