...
which is constructed based on the following rules.
...
We assume these rules are applied in the order in which they are written, for instance, 
...
Without loss of generality, we will assume in the following that all equivalence class of $\Fc$ contain at least one variable,
which comes with no loss of generality since new equalities of the form $y \teq t$ for fresh variable $y$ can be added to $\Fc$ without affecting its satisfiability.
We let $\M( x ) = \Val \ec{x}$ for each variable $x$ in $\Fc$.
...
We show by induction on the size of $\Val \ec{ x }$ that $\M( t ) = \Val \ec{x}$ for each term $t \in \ec{x}$, which implies $\M \models t_1 \teq t_2$.
In the case where $t$ is a variable, then $\M( t ) = \Val \ec{ t } = \Val \ec{ x }$.
In the case where $t$ is a constructor term $C( \vec u )$, 
then $\Val \ec{x}$ must have been assigned by \rn{D-Ind} for some constructor term $C'( \vec u' ) \in \ec{x}$.
...
By our construction, $\M( u ) = C_i( \Val \ec{ x_1 }, \ldots, \Val \ec{ x_{a_i} } )$.
If $i = k$, then $\M( t ) = \M( s^j_k( u ) ) = s^j_k( \M( u ) ) = \Val \ec{x_j}$, which is $\Val( x )$ since $x_j \in \ec{s^j_i( u )} = \ec{x}$.
...
we may interpret $s^j_k( \M( u ) )$ as $\Val \ec{ x }$.
