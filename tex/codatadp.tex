...
The conclusion of a rule either describes equalities to be added to $\Ec$ (in which case we will call an application of it to be \emph{nonterminal}),
...
Together with the rules \rn{Refl}, \rn{Symm}, \rn{Trans}, the rule \rn{Cong} computes the (upward) congruence closure,
while the rules \rn{Unify$_1$} and \rn{Unify$_2$} together compute (downward) unification.
...
\inferrule{
  ...
  \quad
  %\ttpath{\Ec}{\ec{t}}{\ec{t}} \neq \emptyset
  \Val \ec{t_1} = (\vthinspace\mu x.\; t_2)
  \quad
  x \in FV( t_2 )
}{
  \bot
...
no nonredudant application of one of its rules can be applied to $\Ec$.
...
Since \rn{Unify$_2$} does not apply to $\Fc$,
...
By our construction and since \rn{Unify$_1$} and \rn{Unify$_2$} do not apply to $\Fc$, 
