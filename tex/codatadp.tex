...
\def\ATHEN{\ \keyword{then}\ }
\def\AELSE{\untab\qtab\keyword{else}\ }
\def\AFI{\untab}
\def\ARETURN{\keyword{return}\ }
\def\AENDPROC{\untab}
...
infinit\-e constructor terms are also legitimate values of codatatypes
(Section~\ref{sec:theory-of-co-datatypes}). Intuitively, a
codatatype consists of all well-typed finite and infinite ground constructor
terms, and only those. For example, the specification
%
\[\keyw{codatatype}~\,\ty{enat} \,=\, \const{Z} \,\mid\, \const{S}(\ty{enat})\]
%
(using an ML-like syntax) introduces a type that faithfully
models the natural numbers extended with an
infinity $\infty = \const{S}(\const{S}(\const{S}(\ldots)))$. Compared
...
For the datatype case, it
roughly follows the lines of Barrett et al.\ \cite{barrett-et-al-2007}.
To our knowledge, ours is the first decision procedure for codatatypes. 

\newcommand\cheat{\vskip\smallskipamount} %% TYPESETTING
...
\begin{itemize}
\item
\emph{No well-foundedness check is necessary for codatatypes.}
For example, the type of infinite sequences, or streams, over an
element type $\tau$
%
\[\keyw{codatatype}~\;\ty{stream}_{\,\tau} \,=\, \const{SCons}(\tau,\, \ty{stream}_{\,\tau})\]
is allowed; the corresponding datatype would be rejected as
non-well-founded.

\cheat
\item
\emph{A bisimilarity rule takes the place of the cyclicity rule of datatypes.}
...
\cheat
\item
\emph{It must be possible to express cyclic values as closed terms and
to enumerate them.} This is necessary both for finite model finding (modulo theories)
and for theory combinations. The $\mu$-binder notation is used to represent cyclic ($\omega$-regular)
values internally. For example,
the term $\const{SCons}(1,\, \mu s.\; \const{SCons}(0,\, \const{SCons}(9,\, s)))$
denotes the lasso-shaped sequence $1, 0, 9, 0, 9, 0, 9, \ldots$\kern0.1em. %%% TYPESETTING: kern
\end{itemize}
...
widely recognized that decision procedures are more efficient than
axiomatizations.
%axiom.
...
\end{paper}%
The procedure is implemented in the SMT solver CVC4 as rewriting and a theory solver
...
would be easy to support; nesting datatypes inside datatypes, and
...
%we write $\interp{P}$ to denote the

\begin{lemma}[Solution Soundness]
If there exists a derivation tree with root node $\Ec$ containing a saturated node $\Fc$, then $\Ec$ is satisfiable in $\thD$.
\end{lemma}
\begin{proof}
...
The second statement can be assumed for $x : \tau$, since we know that $\tau$ is infinite.
...
interprets wrongly-applied selectors based on information in $\Fc$,
interprets the variables and $x$ of $\Fc$ based on a valuation $\Val$ which construct as follows.
...
Since \rn{Cyclic} does not apply, this definition is well-defined, and since \rn{Cong} does not apply, we have that $\Val{x} = \Val{y}$ if and only if $\ec{x} = \ec{y}$.
For each $e : \tau$, where $\tau \in \Codata$,
\rem{TODO}

We show that all equalities and disequalities in $\Fc$ are satisfied by $\M$.
Say $t_1 \teq t_2 \in \Fc$ where $t_1$ and $t_2$ have type $\tau$ where $\tau \not\in \Codata$,
and assume that $\ec{t_1}$ contains the variable $x$.
We show that $\M( t ) = \Val( x )$ for each term $t \in \ec{x}$.
In the case where $t$ is a variable, then $\M( t ) = \Val( x )$ since $t$ and $x$ are assigned identical values by construction.
In the case where $t$ is a constructor term $C( \vec u )$, say our construction built a valuation for $x$ based on the constructor term $C'( \vec u' ) \in \ec{x}$.
...
By structural induction, we have $\M( \vec u ) = \M( \vec u' )$, and thus $\M( t ) = \M( C( \vec u ) ) = \M( C'( \vec u ' ) ) = \Val( x )$.
In the case where $t$ is a selector term $s^j_k( u )$,
then since \rn{Split} does not apply, 
$\ec{u}$ must contain a term of the form $C_i( s^1_i( u ), \ldots, s^{a_i}_i( u ) )$ for some $i$.
Let $x_1, \ldots, x_n$ be the variables in equivalence classes $\ec{s^1_i( u )}, \ldots, \ec{s^{a_i}_i( u )}$.
By our construction, $\M( u ) = C_i( \Val( x_1 ), \ldots, \Val( x_{a_i} ) )$.
If $i = k$, then $\M( t ) = \M( s^j_k( u ) ) = \Val( x_j )$, which is $\Val( x )$ since $x_j \in \ec{s^j_i( u )} = \ec{x}$.
If $i \neq k$, then $\thD$ does not constraint the value $\M( t )$, and thus we interpret $s^j_k( \M( u ) )$ as $\Val( x )$.

%If $t$ and $s$ are both variables, then this follows since $\ec{t} = \ec{s}$.

Let $t \tneq s \in \Fc$.
Since \rn{Conflict} does not apply, we know $t \teq s \not\in \Fc$ and thus $\ec{t}$ and $\ec{s}$ are distinct.
Since by above, $\M( t ) = \M( s )$ if and only if $\ec{t}=\ec{s}$, we have $\M( t ) \neq \M( s )$, and thus $\M \models t \tneq s$.


By these two, 
%$\M \models_{\thD} \Fc$, and since $\Fc$ is a superset of $\Ec$, we have that $\Ec$ is satisfied by $\M$.
\qed
\end{proof}
