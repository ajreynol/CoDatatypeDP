...
The procedure is complete for ground problems and is compatible with the
...
logical specifications. They can be implemented efficiently and enjoy
properties that can be exploited in automated reasoners. They are especially
...
%
Although they are sometimes perceived as esoteric, codatatypes have a role to
play in computer science. For example, the verified C compiler CompCert
\cite{leroy-2009}, the verified Java compiler JinjaThreads
\cite{lochbihler-2010-jinja}, and the formalized Java memory model
\cite{lochbihler-2014-jmm} all rely crucially on codatatypes to capture infinite
processes.
% as do the CoALP dialect of Prolog and the Dafny specification language.

Codatatypes, like datatypes, are freely generated by their constructors;
but for them, infinite constructor terms are also legitimate values
(Section~\ref{sec:theory-of-co-datatypes}). Intuitively, a
codatatype consists of all finite and infinite ground constructor terms, and
only those. For example, the specification
%
\[\keyw{codatatype}~\,\ty{enat} \,=\, \const{Z} \,\mid\, \const{S}(\ty{enat})\]
%
(using an ML-like syntax) introduces a type that aptly models the natural numbers extended with an
infinite value $\infty = \const{S}(\const{S}(\const{S}(\ldots)))$. Compared
...
Equations such as $\const{S}(\infty) \teq \infty$ hold without any additional
work, because both sides share the same infinite expansion
$\const{S}(\const{S}(\const{S}(\ldots)))$, which uniquely identifies a
codatatype value.

Datatypes and codatatypes are an integral part of most popular proof assistants,
including Agda, Coq, Isabelle, Matita, and PVS. In recent years, more and more
...
In this paper, we introduce a unified decision procedure for ground problems
involving both datatypes and codatatypes. The procedure is described abstractly
as a calculus (Section~\ref{sec:the-calculus}) and is design to be composable
using the Nelson--Oppen method \cite{nelson-oppen-1979}. To our knowledge, this
is the first such decision procedure for codatatypes. For the datatype case, it
roughly follows the lines of Barrett et al.\ \cite{barrett-et-al-2007}.

\newcommand\cheat{\vskip.75\smallskipamount} %% TYPESETTING

Datatypes and codatatypes share many of the same properties, so it makes sense
to consider them together. The main differences are as follows:
%
\begin{itemize}
\item
No well-foundedness check; e.g. the type of infinite sequences, or streams, over an
element type $\tau$
%
\[\keyw{codatatype}~\;\ty{stream}_{\,\tau} \,=\, \const{SCons}(\tau,\, \ty{stream}_{\,\tau})\]
is allowed, whereas the corresponding datatype would be rejected, because no
finite ground constructor terms exist.

\cheat
\item
The cyclicity check must be dropped. A constraint such as
$\const{x} \teq \const{C}(\const{x})$, where $\const{C}$ is a constructor,
is unsatisfiable for datatypes but satisfiable for codatatypes.
Instead: bisimilarity check, which states that two values having the same (possibly
infinite) expansion must be equal. From $\const{x} \teq \const{C}(\const{y})$ and
$\const{y} \teq \const{C}(\const{x})$, deduce $\const{x} = \const{y}$.

\cheat
\item
Normal forms are needed and must be enumeratable. For finite model finding, requires a $\mu$ notation,
e.g., $\const{SCons}(0,\, \mu s.\; \const{SCons}(1,\, \const{SCons}(2,\, s)))$
represents the lasso-shaped sequence $0, 1, 2, 1, 2, \ldots$.
\end{itemize}

Both cyclicity and bisimilarity checks cannot be replaced by finite
axiomatizations, so they really belong in a decision procedure. They are
necessary for completeness of ground problems and for soundness of found models.
...
%    * explain how finite model finding works
 And the other
(co)datatype properties, such as distinctness of constructors and
exhaustiveness, can be done more efficiently by a decision procedure than by an
axiom.
%
The proofs or soundness and completeness are included in the technical report
associated with this paper.

The procedure is implemented in CVC4 as rewriting and a theory solver
(Section~\ref{sec:the-theory-solver}). It implements a precise strategy for applying
inferences. It consists of about 2000 lines of \cpp{} code, among which 1600 are
shared between datatypes and codatatypes. The code is integrated in the
development version of the solver and is expected to be part of the 1.5 release.
%
The decision procedure is evaluated on %hand-crafted examples and on
problems generated from Isabelle theories using Isabelle's Sledgehammer tool
...
(co)recursion, and datatype--codatatype mixtures fall outside this fragment. In
the presentation, we allow ourselves some metalevel parameterization
% at the metalevel
by using subscripts---for example, $\ty{stream}_{\,\tau}$ denotes a
family of ground types $\ty{stream}_{\,\ty{int}}$, $\ty{stream}_{\,\ty{bool}}$,
$\ty{stream}_{\,\ty{stream}_{\,\ty{real}}}$, etc.

Each datatype $\delta$ is equipped by
...
plain, be or be one of the newly introduced (co)datatypes.
%
...
\label{sec:conclusion}
  ...
  * presented a decision procedure for datatypes and codatatypes

  * part of our program to reduce the gap between automatic and interactive
    theorem proving
  * and to enrich provers with more high-level features that can be implemented
    efficiently
