...
\inferrule{
  ...
  \text{ or }
  \bigl( \delta \in \Data \text{ and } \delta \text{ finite} \bigr)
}{
...
from $\tEc$ have a corecursive singleton type
...
the cardinality-1 constraint on the codatatype in \rn{Singleton} are 
...
whose root nodes are the result of applying the rule \rn{Split} on term $t$ of type $\delta$,
...
For instance, the $\mu$-term $\mu x. \const{0}$ representing the integer constant zero
...
%%% At least, that always worked fine for me. :)
%
%\begin{paper} 
...
Thus $t_u\interp{u_i'} \vsim \Val^\ast \ec{t}\interp{ w_i }$. % for each $i = 1, \ldots, n$.
%By Lemma~\ref{lem:mu-cong}, we have $t_u \vsim t_w$.
Thus, $\M( t ) \vsim t_u \vsim \Val^\ast \ec{t}$, and
we have $\M( t ) \vsim \Val^\ast \ec{t}$.

If $t$ is a selector term of the form $\const s^k_j( u )$,
since \rn{Split} does not apply to $\Fc$,
$\ec{u}$ must contain a term of the form $\const C_{j'}( \const s^1_{j'}( u ), \ldots, \const s^{n}_{j'}( u ) )$ for some $j'$.
...
%where $\Val^\ast \ec{u} \interp{ w_i } = \Val^\ast \ec{ \const s^i_{j'}( u )}$ for $i = 1, \ldots, n$. 
Since \rn{Inject} and \rn{Clash} do not apply to $\Fc$, 
...
If $j = j'$, then $\M( t )$ is $\alpha$-equivalent with $\Val^\ast \ec{u} \interp{w_k}$, which is equal to $\Val^\ast \ec{ \const s^k_j( u )}$,
which is $\Val^\ast \ec{t}$.
If $j \neq j'$, we are free to interpret $\const s^k_j( \M( u ) )$ as the value in $\M( \tau )$ that is $\alpha$-equivalent with $\Val^\ast \ec{ t }$.
