...
For acyclicity, if $\ec{t : \delta}$ is an equivalence class such that $\delta$ is a datatype and
...
For uniqueness, if $\ec{t : \delta}$ and $\ec{u : \delta}$ are two equivalence classes such
...
$\muvar( t ) = \begin{cases}
  x & \text{if $t = \MU {x}.\; u$} \\[-\jot]
  t & \text{otherwise ($t$ is a variable)}
\end{cases}$
...
and $\Val^\AST$ is a completion of $\Val$ for normal model $\M$, the following properties hold:
\begin{enumerate}
...
%By Lemma~\ref{lem:model-completion}\PART{2} and due to our construction of $\Val^\AST$, we have $\Val^\AST \ec{t}$ is a term
...
%By Lemma~\ref{lem:model-completion}\PART{2} and due to our construction of $\Val^\AST$, we have $\Val^\AST \ec{u}$ is of the form
%$\MU {\Varec{u}}.\; \const C_{\negvthinspace\jPrime}( w_1, \ldots, w_n )$,
%where $\Val^\AST \ec{u} \interp{ w_i } = \Val^\AST \ec{ \const s^i_{\negvthinspace\jPrime}( u )}$ for $i = 1, \ldots, n$. 
Since \rn{Inject} and \rn{Clash} do not apply to $\Fc$, 
...
$\Val^\AST \ec{u} \interp{ w_i } = \Val^\AST \ec{ \smash{\const s^i_{\negvthinspace\jPrime}( u )}}$. 
If $j = \jPrime$, then $\M( t )$ is $\alpha$-equivalent with $\Val^\AST \ec{u} \interp{w_k}$, which is equal to $\Val^\AST \ec{ \smash{ \const s^k_{\negvthinspace j}( u )}}$,
