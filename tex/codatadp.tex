...
A possible sequence of \rn{Update} rule applications is presented below.
\[\begin{array}{l@{\hspace{1em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l}
%\hline
%\text{Step} & \multicolumn{3}{l}{\text{Mapping }\Val}
%\MIDRULE
\text{1. Initial:}
& \{ & \ec{x} \mapsto \Var{\ec{x}}, & \ec{z} \mapsto \Var{\ec{z}}, & \\
&    & \ec{y} \mapsto \Var{\ec{y}}, & \ec{w} \mapsto \Var{\ec{w}} & \} \\[\jot]
\text{2. Update w.r.t.\ }\ec{x}\text{:}
& \{ & \ec{x} \mapsto \mu \Var{\ec{x}}. \const{C}( \Var{\ec{w}}, \Var{\ec{y}} ), & \ec{z} \mapsto \Var{\ec{z}}, & \\
&    & \ec{y} \mapsto \Var{\ec{y}}, & \ec{w} \mapsto \Var{\ec{w}} & \} \\[\jot]
\text{3. Update w.r.t.\ }\ec{y}\text{:}
& \{ & \ec{x} \mapsto \mu \Var{\ec{x}}. \const{C}( \Var{\ec{w}}, \mu \Var{\ec{y}}. \const{C}( \Var{\ec{z}}, \Var{\ec{x}} ) ), & \ec{z} \mapsto \Var{\ec{z}}, & \\
&    & \ec{y} \mapsto \mu \Var{\ec{y}}. \const{C}( \Var{\ec{z}}, \Var{\ec{x}} ), & \ec{w} \mapsto \Var{\ec{w}} & \} \\[\jot]
\text{4. Update w.r.t.\ }\ec{x}\text{:}
& \{ & \ec{x} \mapsto \mu \Var{\ec{x}}. \const{C}( \Var{\ec{w}}, \mu \Var{\ec{y}}. \const{C}( \Var{\ec{z}}, \Var{\ec{x}} ) ), & \ec{z} \mapsto \Var{\ec{z}}, & \\
&    & \ec{y} \mapsto \mu \Var{\ec{y}}. \const{C}( \Var{\ec{z}}, \mu \Var{\ec{x}}. \const{C}( \Var{\ec{w}}, \Var{\ec{y}} ) ), & \ec{w} \mapsto \Var{\ec{w}} & \}
%\hline
\end{array}\]
%The first column shows the equivalence class that was considered;
%the second column shows the content of $\Val$.
...
%and $\muvar( w_i ) = \Var{\ec{t_i}}$ for $i = 1, \ldots, n$,
%\item
%$t_1 \interp{ t_2 } = \Val^\ast \ec{x}$
%for all $t_1$ with subterm $t_2$ in the range of $\Val^\ast$ where $\muvar(t_2)=\Var{\ec{x}}$,
\item 
...
%Each $w_i$ is either $\Var{\ec{x}}$,
%a variable $\Var{\ec{y}}$ where $\ec{y}$ does not contain a constructor,
%or a term of the form $\mu \Var{\ec{y}}. \const{C}_j( \vec u )$.
While interpreting $w_i$ in $\Val \ec{x}$,
all free occurrences of $\Var{\ec{x}}$ in $\Val \ec{x} \interp{w_i}$ are replaced by $\Val \ec{x}$. \rem{better formalize.}
...
%$\Val^\ast \ec{ u }$ must be of the form $t_u = \mu \Var{\ec{u}}. C_i( u_1, \ldots, u_n )$,
%where $\muvar( u_1 ) = \Var{\ec{s^1_i(u)}}$, $\ldots$, $\muvar(u_n) = \Var{\ec{s^n_i( u )}}$,
