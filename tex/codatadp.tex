...
properties of constructors and selectors. All properties below range over all
$i$, $j$, $\jPrime$, and $k$ within bounds and to all possible splits of the
...
\text{Distinctness:}\quad
  & \forall \bar x, \bar y.\;\, \const{C}_{i\negvthinspace j}(\bar x) \tneq \const{C}_{i\negvthinspace \jPrime}(\bar y) \qquad\text{if $j \not= \jPrime$}
  \\
\text{Injectivity:}\quad
  & \forall \bar x, y, \bar z.\;\, \const{C}_{i\negvthinspace j}(\bar x, y, \bar z) \teq \const{C}_{i\negvthinspace j}(\bar x, y', \bar z) \longrightarrow y \teq y'
  \\
...
the source of an edge in $p$ or does not contain an application of a constructor (including the case when $t$ is not of datatype/codatatype type).
