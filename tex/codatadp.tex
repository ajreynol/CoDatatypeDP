...
\item 
for all selector terms $s^j_i( t )$ of type $\tau$ where $\M( t )$ is of the form $\mu x. \const{C}_i( \vec u )$.% for some $\vec u$,
...
The \emph{completion} of $\Val$, which we will write as $\Val^\ast$, 
...
\inferrule{
  ...
  c \in \M( \tau )
  \quad
  c \not\sim_x \mu y. t \text{ for all } \mu y. t \in \mathcal{T}(\varna\Val)
}{
  \Val := \Val \{ x \mapsto c \}
}
\)

\rem{revise from here on}
...
%Since $\nf{(t_2 \interp{\mu y. u})} \in \mathcal{T}( t_2 )$.
\end{proof}
