...
\)
\end{tabular}
\caption{Rules for bidirectional congruence closure.
}
\label{fig:cc-rules}
\end{figure}

Following the conventions from~\cite{}, 
the derivation rules of our calculus are given in Figures~\ref{fig:cc-rules},~\ref{fig:ab-rules}, and~\ref{fig:split-rule} in \emph{guarded assignment form},
...
Figure~\ref{fig:cc-rules} gives the rules for constructing the (bidirectional) congruence closure of $\Ec$. \rem{more?}

Assume that $\Ec$ is saturated with respect to the rules in Figure~\ref{fig:cc-rules}, that is,
...
First, an edge $\ec{t_1} \rightarrow^{C}_n \ec{t_2}$ is a transition between two equivalence classes $\ec{t_1}$ and $\ec{t_2}$ (which we will call its source and destination respectively),
labelled by a constructor $C$ and integer $n$.
We say that $\Ec$ \emph{induces edge} $\ec{t_1} \rightarrow^{C}_n \ec{t_2}$ if and only if $C( \vec u ) \in \ec{t_1}$ for some $\vec u$ and $\ec{t_2} = \ec{u_n}$.
...
We say that $\Ec$ \emph{induces path} $p$ if it induces each of its edges.
Notice that since $\Ec$ contains a finite set of equivalence classes, all paths induced by $\Ec$ are finite.
The set $\ttpath{\Ec}{\ec{t_1}}{\ec{t_2}}$ is the smallest set containing all induced paths starting with source $\ec{t_1}$ and destination $\ec{t_2}$.
