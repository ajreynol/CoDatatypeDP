...
The content of set $\mathcal{T}^\M_{v_{\ec{x}}}( \Val )$ is $\alpha$-equivalent to
\[\begin{array}{c}
\{
...
\}
\end{array}\]
Say that $\M$ interprets variables in $\Ec$ based on the mapping $\Val$,
...
then the values in the range of $\Val$ remain distinct.
...
We only briefly mention these optimizations, since each applies to datatypes as well as co-datatype, and most have been presented thoroughly in~\cite{}.
...
Handling discriminators requires extending the decision procedure with several additional rules (see~\cite{} for more details), which apply uniformly to both datatypes and co-datatypes.
...
In particular, we rewrite $t \teq s$ to $s \teq t$ if $s$ is less than $t$ based on a term ordering,
...
reports that a subset $C$ of $M_i$ is unsatisfiable in which case $\neg C$ is added as a additional clause to $F$,
...
The rules of the calculus from Section~\ref{sec:the-calculus} are implemented as follows.
First, for each equality, we associate an \emph{explanation} for that equality.
\rem{TODO}

Step 1 of the calculus (along with the optimization for collapsing selectors) and the \rn{Singleton} rule are performed eagerly, that is for partial satisfying assignments $M$, while step 2 and the \rn{Split} rule from step 3 are performed only for complete satisfying assignments $M$, and in that order.
...
No (co)datatypes
  & 221 & 760 & \phantom{0}36 & 1017 \\
Datatypes without cyclicity
  & 227 & 765 & \phantom{0}36 & 1028 \\
Datatypes with cyclicity
  & 227 & 771 & \phantom{0}36 & 1034 \\
Codatatypes without bisimilarity
  & 217 & 787 & \phantom{0}34 & 1038 \\
Codatatypes with bisimilarity
  & 218 & 787 & \phantom{0}36 & 1041 \\
Full (co)datatypes
  & 224 & 798 & \phantom{0}36 & 1058 \\[\jot]
