...
\AXC{\strut$R_i[v_i, w_i]$}
  \AXC{\strut$\forall v\; w.\;\, R_i[v, w] \longrightarrow \bigwedge_k \const{s}_{i\negvthinspace j}^{\,k}(v) \sim \const{s}_{i\negvthinspace j}^{\,k}(w)$\quad for each $i$}
\BIC{\strut$v_1 \teq w_1 \mathrel\land \cdots \mathrel\land v_l \teq w_l$}
\DP
\end{aligned}
\]
