...
and let $\tcEc$ be the set of subterms of $\Ec \mathrel\cup \{ \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) ) \mid t : \delta \in \SSS \mathrel\cup \SSSS \ast,\, \const C_i \in \Ctr^\delta \}$.
...
For \rn{Acyclic}, the construction of $\Val$ indicates that the class of values that $t_1$ can take in models of $\Ec$ is infeasible,
...
by the induction hypothesis $\Ec \mathrel\cup t \teq \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) )$ is unsatisfiable
