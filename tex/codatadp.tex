...
$\mu v_{\ec{x}}. \const{C}( \mu x_1. \const{A}( x_1) )$, or
$\mu v_{\ec{x}}. \const{B}( \const{A}( v_{\ec{x}} ) )$
to $v_{\ec{x}}$.
...
$\Val \ec{w_1}$ is $\mu v_{\ec{w_1}}. \const{A}( \mu v_{\ec{x}}. \const{B}( \const{A}( v_{\ec{x}} ) ) )$.
$\Box$
\end{example}
...
\begin{enumerate}
\item $\Val^\ast \ec{x}$ is $\alpha$-equivalent to a value in $\M( \tau )$, for all $\ec{x}$ of type $\tau$ of $\Ec$,
\item
...
The rules in step 1 of the calculus (along with the optimization for collapsing selectors) and the \rn{Singleton} rule are performed eagerly, 
that is for partial satisfying assignments $M$, while the rules in step 2 and the \rn{Split} rule are performed only for complete satisfying assignments $M$, 
and in that order.
