...
\begin{aligned}[t]
\text{Induction:}\quad
\AXC{$\forall x_1 \ldots \vthinspace x_{n_{ij}}.\; \bigl(\bigwedge_{\,k\,} \mathit{IH}_{ij}^k[\bar x_k]\bigr) \longrightarrow P_i[C_{ij}(\bar x)]$\quad\text{for each $i,j$}}
\UIC{$P_1[v_1] \mathrel\land \cdots \mathrel\land P_l[v_l]$}
\DP
\end{aligned}
\]
where the induction hypothesis $\mathit{IH}_{ij}^k(x)$
denotes either $P_{i'}(x)$ for some $i$ such that this is type
correct or else $\top$.
%
...
\AXC{for each i: $(\forall x y. R_i[x, y] \longrightarrow \bigwedge_k s_{ij}^k(x) \sim s_{ij}^k(y))$}
