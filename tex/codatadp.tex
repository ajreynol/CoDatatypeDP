...
but for codatatypes, infinite constructor terms also denote values
...
work, because both sides of the equality share the same infinite expansion
$\const{S}(\const{S}(\const{S}(\ldots)))$.
...
In this paper, we introduce a unified decision procedure for both datatypes and
codatatypes. The procedure is described abstractly as a calculus
(Section~\ref{sec:the-calculus}). To our knowledge, this is the first decision
procedure for codatatypes. For the datatype case, it roughly follows the lines
of Barrett et al.\ \cite{barrett-et-al-2007}. Because datatypes and codatatypes
share many of the same properties, it is sensible to consider them together.
