...
non-well-founded dual of (inductive) \relax{datatypes}.
%
...
the term $\const{SCons}(1,\: \mu\vthinspace s.\; \const{SCons}(0,\: \const{SCons}(9,\: s)))$
...
induction rules out nonstandard models, which could contain cyclic values---e.g.,
...
such~as
\[\begin{aligned}[t]
      \keyw{datatype}\;\, \ty{ext\_nat} & \,=\, \const{Nat}(\ty{nat}) \,\mid\, \const{Infty} \\[-.5\jot]
      \keyw{codatatype}\; \,\ty{complex} & \,=\, \const{Complex}(\const{re}{:}\; \ty{real},\; \const{im}{:}\;\ty{real})
\end{aligned}
\]
At the semantic level, it makes no difference whether the types are introduced as
datatypes or as codatatypes.
...
infinite: \,$\mu\vthinspace a.\; A(a)$. Other specimens are
\[\begin{aligned}[t]
...
precisely in the models that interpret \ty{unit} as a singleton. Fortunately,
given some cardinalities for the ordinary types, it is easy to characterize
this degenerate case:

% In other words, all finite corecursive codatatypes are corecursive singletons.

\begin{lemma}
Let $\delta$ be a corecursive codatatype. If the domain interpreting $\delta$
is finite, it must be a singleton. Furthermore, $\delta$ has necessarily a
...
that the type has only one value.
\qed
\end{proof}
...
%We first show that for any saturated node $\Fc$, 
%additional equalities $\Gc$ can be added to $\Fc$ such that each codatatype/datatype equivalence class of $\Fc \cup \Gc$ has a constructor term.
%For this purpose, we extend the calculus in this section.
%For a subset of equivalence classes $S$ of $\Ec$, write $\dpath{\Ec}{C,n}{S}$ to denote the set
%such that $e' \in \dpath{\Ec}{C,n}{S}$ if and only if there exists an $e \in S$ such that $e \rightarrow^{C}_n e'$ is induced by $\Ec$.
...
\inferrule{
  ...
  \Val := \Val \{ v_{\ec{x}} \mapsto \mu\vthinspace v_{\ec{x}}. C( v_{\ec{t_1}}, \ldots, v_{\ec{t_n}} ) \}
}
\)
...
\rem{define $\sim$}
We assume that first \rn{Ind} is applied exhaustively, after which \rn{Base} is applied exhaustively.
%the rule \rn{D-Ind} is applied to completion before applying \rn{D-Base}.
In the rule \rn{Ind}, we write $FV( \Val )$ to denote the free variables occurring in $\Val$.
...
\rn{Ind} \ec{x}
& \{ & \ec{x} \mapsto \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, v_{\ec{y}} ), & \ec{z} \mapsto v_{\ec{z}}, \\
&    & \ec{y} \mapsto v_{\ec{y}}, & \ec{w} \mapsto v_{\ec{w}} & \} \\
\rn{Ind} \ec{y}
& \{ & \ec{x} \mapsto \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, \mu\vthinspace v_{\ec{y}}. \const{C}( v_{\ec{z}}, v_{\ec{x}} ) ), & \ec{z} \mapsto v_{\ec{z}}, \\
&    & \ec{y} \mapsto \mu\vthinspace v_{\ec{y}}. \const{C}( v_{\ec{z}}, v_{\ec{x}} ), & \ec{w} \mapsto v_{\ec{w}} & \} \\
\rn{Ind} \ec{x}
& \{ & \ec{x} \mapsto \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, \mu\vthinspace v_{\ec{y}}. \const{C}( v_{\ec{z}}, v_{\ec{x}} ) ), & \ec{z} \mapsto v_{\ec{z}}, \\
&    & \ec{y} \mapsto \mu\vthinspace v_{\ec{y}}. \const{C}( v_{\ec{z}}, \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, v_{\ec{y}} ) ), & \ec{w} \mapsto v_{\ec{w}} & \} \\
\rn{Base} \ec{z}
& \{ & \ec{x} \mapsto \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, \mu\vthinspace v_{\ec{y}}. \const{C}( \const{0}, v_{\ec{x}} ) ), & \ec{z} \mapsto \const{0}, \\
&    & \ec{y} \mapsto \mu\vthinspace v_{\ec{y}}. \const{C}( \const{0}, \mu\vthinspace v_{\ec{x}}. \const{C}( v_{\ec{w}}, v_{\ec{y}} ) ), & \ec{w} \mapsto v_{\ec{w}} & \} \\
\rn{Base} \ec{w}
& \{ & \ec{x} \mapsto \mu\vthinspace v_{\ec{x}}. \const{C}( \const{1}, \mu\vthinspace v_{\ec{y}}. \const{C}( \const{0}, v_{\ec{x}} ) ), & \ec{z} \mapsto \const{0}, \\
&    & \ec{y} \mapsto \mu\vthinspace v_{\ec{y}}. \const{C}( \const{0}, \mu\vthinspace v_{\ec{x}}. \const{C}( \const{1}, v_{\ec{y}} ) ), & \ec{w} \mapsto \const{1} & \} \\
\end{array}\]
...
\label{sec:experimental-results}
  ...
  * to evaluate procedure, we generated problems from Isabelle theories using
    Sledgehammer

  * chose a selection of theory files:
    * pretty much every theory file from Isabelle distribution and AFP that
      defines a codatatype
    * also included two private theories by Andreas Lochbihler and Peter Gammie
    * (codatatypes being a recent addition to Isabelle \cite{itp},
       they are not yet widely used)
    * and included a number of theories on datatypes
    * aimed at a good mixture between linear structures (lists, streams) and
      tree structures
    * the theory files were chosen before running the experiments

  * for each goal, select 256 lemmas as axioms, before monomorphization
    (after monomorphization, typically more formulas due to duplicates),
    and encode with the goal

  * then ran CVC4 for XXX seconds on the StarExec cluster \cite{xxx}

{\footnotesize
\begin{verbatim}
For the evaluation of Section 5, I'm playing with various ideas. I certainly
would like to keep it short and not waste too much of our time on it. We
discussed earlier having (1) hand-crafted problems and (2) actual benchmarks
(like those you already have). I suggest we drop (1); I never did hand-crafted
before and this always looks suspicious anyway. For (2), I see three or four
possible benchmark sets:

1. without (co)datatype theories
2. with datatypes
(3. with codatatypes)
4. with both

The benchmark problems would be identical except for the declarations at the
top ("declare-sort" and "declare-fun" vs. "declare-(co)datatype").
Irrespective of whether interpreted (co)datatypes are used, Sledgehammer will
typically include some (co)datatype axioms (i.e. tautologies if (co)datatypes
are enabled). Hence the tests are a good indication of whether native
(co)datatypes are useful to Sledgehammer. I assume the results will follow
common sense, i.e. 4 > {3, 2} > 1

As for the actual Isabelle theory files to use, for codatatypes there is not
much question: We just use all I could get my hands on. But perhaps we should
have a second benchmark set, ideally something a bit standard like, say, the
Judgment Day (JD) suite. Or I can make some arbitrary a priori (before
evaluation) decisions, but that always looks a bit fishy unless it can be
justified easily. The trouble is, we have about 1000 datatypes lying around in
Isabelle and the Archive of Formal Proofs (AFP). These would have no
codatatypes in them, so we could only do 1 and 2 of the evaluation. We could
at least try the "list" library, which is extremely developed and would have
1600 goals already, and perhaps a few really basic types ("option", "string"),
or perhaps red-black trees (about 450 goals).

So we can show the success rates, in a table like this:

                 Codatatype-oriented suite      Datatype (or JD) suite
                      (2345 goals)                   (1240 goals)

   w/o (co)data        1255                            699
   w data              1268                            712
   w codata            1270
   w both              1273

I can write the usual warning that an increase of a couple of percentage
points in success rate in a general tool like Sledgehammer and general
benchmark suites means a lot etc. Putting the datatype evaluations (i.e. 2 and
4) in there offers the advantage that it will give some measure of how much to
expect from the codatatypes. I.e. if the success rate doesn't go up by 50% for
datatypes, nobody will expect this to happen for codatatypes. A priori, I
would expect a similar gain, but I'm looking forward to see the experimental
data.

In addition to success rate (or number of solved goals), we should have other
statistics, like how often do we get proofs that contain the cyclicity and
bisimilarity rules. And finally, we look in more detail at one or two such
proofs, by presenting it and explaining it (and, before that, understanding
it).
\end{verbatim}}
...
provers with high-level features and at reducing the gap between automatic and
...
\cite{blanchette-nipkow-2010}. We also see some opportunities to enrich
SMT solvers with recursive and corecursive functions. Finally, it might be
possible to go further in terms of supporting nested and mixed (co)recursion
and quantified formulas over (co)datatypes in solvers.
