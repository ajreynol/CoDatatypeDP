...
for $i \geq 0$ let $S^{i+1}_t =$ $S^i_t \mathrel\cup \{ s( u ) \mid u : \upsilon \in S^i, \upsilon \in \Data, \upsilon \text{ finite}, s \in \Sel^{\upsilon}  \}$,
...
and let $\tcEc$ be the set of subterms of $\Ec \mathrel\cup \{ \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) ) \mid t : \tau \in S \mathrel\cup S^\ast, \const C_i \in \Ctr^\tau \}$.
