...
%
\begin{enumerate}
\item \noindent\rlap{Initial state:}\phantom{Unfold $\Varec{x}$:\enskip}$\{  \}$
\item \noindent\rlap{Unfold $\Varec{x}$:}\phantom{Unfold $\Varec{x}$:\enskip}$\{\vthinspace  \ec{x} \mapsto \mu {\Varec{x}}.\; \const{C}( {\Varec{w}},\, {\Varec{y}} ) \}$
...
\begin{example}
Let $\const{C} : $ $\tau_1 \times \tau_2 \rightarrow \tau_2$ be a constructor,
...
\inferrule{
  t : \tau \in \tEc
  \quad
  ( \const s( t ) \in \tEc \text{ and } \const s \in \Sel^\tau )
  \text{ or }
