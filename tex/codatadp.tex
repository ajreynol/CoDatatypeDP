...
\begin{lemma}
If the domain interpreting a corecursive codatatype is finite, it must be $1$.
\end{lemma}
...
necessarly one constructor and that all arguments have types of cardinality 1.
%
Sometimes, a type might be a corecursive singleton only in some models. For
example, if \ty{unit} is an uninterpreted type, \ty{b} and \ty{c} will be
singleton types precisely in the models that interpret \ty{unit} as a
singleton.
...
contribution has been the addition of codatatypes. These are not very
difficult per se, but there are tricky corner cases to take care of if we care
about completeness (which we do). On the practical side, we obtained
