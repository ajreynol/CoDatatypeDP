...
By induction on the size of $\Val( x )$, we show that $\M( t ) = \Val( x )$ for each term $t \in \ec{x}$, which implies $\M \models t_1 \teq t_2$.
...
By the induction hypothesis, we have $\M( \vec u ) = \M( \vec u' )$, and thus $\M( t ) = C( \M( \vec u ) ) = C'( \M( \vec u ' ) ) = \Val( x )$.
...
we may interpret $s^j_k( \M( u ) )$ as $\Val( x )$.
...
By Lemmas~\ref{lem:t},~\ref{lem:rs}, and~\ref{lem:ss}, our calculus is sound and complete for $\thD$.
