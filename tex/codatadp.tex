...
then since \rn{Unify$_2$} does not apply to $\Fc$,
...
Since \rn{Unify$_1$} does not apply to $\Fc$, it must be that $\ec{u_i} = \ec{y_i}$ for $i = 1, \ldots, n $.
%Due to Lemma~\ref{lem:model-completion}.2 and by induction
\rem{TODO}
...
Peter Gammie and Andreas Lochbihler (\gandl). We complemented the benchmarks
...
tautologies. This allows us to answer the question, \relax{What are the
