...
$\MU {\Varec{u}}.\; \const C_l( w_1, \ldots, w_n )$,
where $\Val^\ast \ec{u} \interp{ w_j } = \Val^\ast \ec{ \const s^j_k( u )}$. 
If $k = l$, then $\M( t )$ is $\alpha$-equivalent with $\Val^\ast \ec{u} \interp{w_j} = \Val^\ast \ec{ \const s^j_k( u )}$,
which is $\Val^\ast \ec{t}$.
If $k \neq l$, we interpret $\const s^j_k( \M( u ) )$ as the value in $\M( \tau )$ that is $\alpha$-equivalent with $\Val^\ast \ec{ t }$.
