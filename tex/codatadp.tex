...
The sets $\tpath{\Ec}{\ec{x}}$ and $\tpath{\Ec}{\ec{\const{C}(y,x)}}$ are:
\[\begin{array}{l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l@{\hspace{.5em}}l}
\{ & \ec{x} \rightarrow^{\const{C}}_1 & \ec{y}, & & & 
\{ & \ec{\const{C}(y,x)} \rightarrow^{\const{C}}_1 & \ec{y}, & & \\ 
   & \ec{x} \rightarrow^{\const{C}}_2 & \ec{\const{C}(y,x)} \rightarrow^{\const{C}}_1 & \ec{y},&  &
   & \ec{\const{C}(y,x)} \rightarrow^{\const{C}}_2 & \ec{x} \rightarrow^{\const{C}}_1 & \ec{y},& \\
   & \ec{x} \rightarrow^{\const{C}}_2 & \ec{\const{C}(y,x)} \rightarrow^{\const{C}}_2 & \ec{x} & \} &
   & \ec{\const{C}(y,x)} \rightarrow^{\const{C}}_2 & \ec{x} \rightarrow^{\const{C}}_2 & \ec{\const{C}(y,x)} & \}.
\end{array}\]
Thus, by the rule \rn{Bisimilar}
for the mapping $\sigma :=$ $\{ \ec{x} \mapsto \ec{\const{C}(y,x)}, \ec{\const{C}(y,x)} \mapsto \ec{x} \}$,
we may infer $x \teq \const{C}(y,x)$.
\qed
\end{example}
...
\footnote{We assume all such values are well-typed.}, or a variable $x$.
...
To reason about the uniqueness of $\mu$-values, we introduce the following notions.
First, the \emph{interpretation of subvalue $u$ in $v$} is the value $v\interp{u}^\emptyset$ returned by the following,
...
v\interp{x}^M & = & x & x \in M \\
v\interp{x}^M & = & v\interp{\mu x. C( \vec t )}^M & x \not\in M, x \mapsto \mu x. C( \vec t ) \in v  \\
v\interp{\mu x. C( \vec t )}^M & = & x & x \in M \\
v\interp{\mu x. C( \vec t )}^M & = & \mu x. C( v\interp{ \vec t }^{M \cup \{ x \}} ) & x \not\in M
\end{array}\]

We say a (closed) $\mu$-value $v$ of the form $\mu x. \const{C}( \vec w )$ is \emph{normal} if and only if 
$v\interp{u}^\emptyset$ is not equivalent to $v$ modulo renaming for all of its strict subvalues $u$,
and all of its strict closed subvalues are normal.
