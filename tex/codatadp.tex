...
and all of its strict subterms that do not contain $x$ are normal.
...
(1) unique up to renaming of $\mu$-bound variables,
(2) acyclic when $\tau \not\in \Codata$.
...
interprets the variables of $\Fc$ based on the valuation $\Val$ which was constructed in step two of our calculus.
...
\inferrule{
  v_{\ec{x}} \in FV( \Val )
  \quad
  c_1 \text{ closed normal}  
  \quad
  c_1 \not\sim c_2 \text{ for all } c_2 \in \mathcal{T}(\Val)
}{
  \Val := \Val \{ v_{\ec{x}} \mapsto c_1 \}
}
\)
...
Let $\Val^\ast$ be the result of applying the above rule to completion on the $\Val$ we constructed during step 2 of our calculus.
...
there exists a closed normal $\mu$-term $c_1$ that is not bisimilar to any $c_2 \in \mathcal{T}(\Val^\ast)$, since $\mathcal{T}(\Val^\ast)$ is a finite set. \rem{elaborate?}
...
\end{eqnarray}
We show this by induction on the number of applications of the rule \rn{Complete}.
For the base case, note that since \rn{Bisimilar} does not apply to $\Fc$,
...
For the inductive case, say that we apply \rn{Complete} to $\Val$ for the substitution $\sigma := \{ v_{\ec{x}} \mapsto c_1 \}$.
\rem{TODO}


Now, we let $\M( x ) = \Val^\ast \ec{x}$ for each variable $x$ in $\mathcal{T}(\Fc)$.
...
If $\tau \in \Data$,
we first show that $\M( t ) = \Val^\ast \ec{x}$ for all non-selector terms $t$ that occur in datatype equivalence classes $\ec{x}$.
