...
\[\begin{array}{rcrlrl}
\Val \ec{x} & \,=\, &
\mu {\Varec{x}}. & \const{C}( {\Varec{y}},\, & \smash{\mu {\vthinspace\VAREC{\const{C}(y, x)}}}. & \const{C}( {\Varec{y}},\, {\Varec{x}} ) ) \\
\Val \ec{\const{C}(y,x)} & = &
\mu {\vthinspace\VAREC{\const{C}(y,x)}}. & \const{C}( {\Varec{y}},\, & \mu {\Varec{x}}. & \const{C}( {\Varec{y}},\, {\VAREC{\const{C}(y,x)}} ) )
\end{array}\]
...
the equivalence class of $t$ must contain an application of one of the constructors of $\delta$ (where $\Ctr^\delta = \{ \const{C}_1, \ldots \const{C}_m \}$).
