...
\begin{aligned}[t]
\text{Induction:}\quad
for each i, j: (\forall \bar x. IH_{ij}[\bar x] --> P_i(C_{ij}(\bar x)))
---------------------------
P_1[x_1] \& ... \& P_l[x_l]
\end{aligned}
\]
...
\begin{aligned}[t]
\text{Conduction:}\quad
for each i: R_i[x_i, y_i]
for each i: (\forall x y. R_i[x, y] --> \bigwedge_k s_{ij}^k(x) \sim s_{ij}^k(y))
---------------------------------
x_1 \teq y_1 & ... & x_l \teq y_l
\end{aligned}
\]
