...
interprets the variables and $x$ of $\Fc$ based on a valuation $\Val$, which is constructed as follows.
...
Since \rn{Cong} does not apply, we have that:
...
If $\tau \in \Nondata$, then $\M( t_1 ) = \M( t_2 )$ by construction.

If $\tau \in \Data$, then assume the equivalence class of $t_1$ and $t_2$ contains variable $x$.
We show that $\M( t ) = \Val( x )$ for each term $t \in \ec{x}$, which implies $\M \models t_1 \teq t_2$.
...
If $i = k$, then $\M( t ) = \M( s^j_k( u ) ) = s^j_k( \M( u ) ) = \Val( x_j )$, which is $\Val( x )$ since $x_j \in \ec{s^j_i( u )} = \ec{x}$.
If $i \neq k$, then $\thD$ does not constraint the value of $\M( t )$ and since $s^j_k( \M( u ) )$ is otherwise unconstrained in our model construction since \rn{Cong} does not apply, 
we interpret $s^j_k( \M( u ) )$ as $\Val( x )$.

If $\tau \in \Codata$, \rem{TODO}

We show that all disequalities in $\Fc$ are satisfied by $\M$.
