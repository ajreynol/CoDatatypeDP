...
$\M( C( \vec t ) )$ for $C \in \Ctr$ is the value in $\M( \tau )$ that is a renaming of $\nf{(\mu x. C( \M( \vec t ) ))}$ where $x \not\in FV( \vec t )$.
The interpretation of (correctly-applied) selector terms
$\M( s^j_i( t ) )$ for $s^j_i \in \Sel$ is the value in $\M( \tau )$ that is a renaming of $\M( t )\interp{u_j}$
...
there exists a closed normal $\mu$-term $c_1$ that is not bisimilar to any $c_2 \in \mathcal{T}(\Val^\ast)$, 
since $\mathcal{T}(\Val^\ast)$ is a finite set (noting that a closed $\mu$-term is only bisimilar to other closed $\mu$-terms).
