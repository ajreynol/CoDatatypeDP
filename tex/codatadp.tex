...
%        consider bisimilarity -- no way to express that certain infinite
...
\paragraph{Phase 2: Applying Acyclicity and Bisimilarity.}
...
A possible sequence of \rn{Update} rule applications is presented below, omitting
trivial entries such as $\ec{w} \mapsto {\Varec{w}}$.
%
\begin{enumerate}
\item \noindent\rlap{Initial state:}\phantom{Update w.r.t.\ $\ec{x}$:\enskip}$\{  \}$
\item \noindent\rlap{Update w.r.t.\ $\ec{x}$:}\phantom{Update w.r.t.\ $\ec{x}$:\enskip}$\{\vthinspace  \ec{x} \mapsto \mu {\Varec{x}}.\; \const{C}( {\Varec{w}},\, {\Varec{y}} ) \}$
\item \noindent\rlap{Update w.r.t.\ $\ec{y}$:}\phantom{Update w.r.t.\ $\ec{x}$:\enskip}$\{\vthinspace  \ec{x} \mapsto \mu {\Varec{x}}.\; \const{C}( {\Varec{w}},\; \mu {\Varec{y}}.\; \const{C}( {\Varec{z}},\, {\Varec{x}} ) ),\;
  \ec{y} \mapsto \mu {\Varec{y}}. \const{C}( {\Varec{z}},\, {\Varec{x}} ) \vthinspace\}$
\item \noindent\rlap{Update w.r.t.\ $\ec{x}$:}\phantom{Update w.r.t.\ $\ec{x}$:\enskip}$\{\vthinspace  \ec{x} \mapsto \mu {\Varec{x}}.\; \const{C}( {\Varec{w}},\; \mu {\Varec{y}}.\; \const{C}( {\Varec{z}},\, {\Varec{x}} ) ),\;
...
are of the form $\const{C}( {\Varec{w}}, \const{C}( {\Varec{z}}, \ldots ))$
and $\const{C}( {\Varec{z}}, \const{C}( {\Varec{w}}, \ldots ))$,
respectively, for some value of ${\Varec{w}}$ and ${\Varec{z}}$.
\end{example}
...
The \rn{Singleton} rule handles the case when there are two terms from $\tEc$ having a co-recursive singleton type,
...
First, if the derivation tree is an application of \rn{Conflict}, \rn{Clash}, or \rn{Acyclic},
...
For \rn{Acyclic}, our construction of $\Val$ indicates that the class of values that $t_1$ can take in models of $\Ec$ is infeasible,
...
whose root node is $\Ec \cup t_1 \teq t_2$ obtained by applying one of the rules \rn{Refl}, \rn{Sym}, \rn{Trans}, \rn{Cong}, \rn{Inject}, \rn{Bisimilar}, or \rn{Singleton}.
...
$\mu {\Varec{x}}. \const{C}( \mu x_1. \const{A}( x_1) )$, and
$\mu {\Varec{x}}. \const{B}( \mu x_1. \const{A}( {\Varec{x}} ) )$
to ${\Varec{x}}$.
...
$\Val \ec{w_1}$ is $\mu {\Varec{w_1}}. \const{A}( \mu {\Varec{x}}. \const{B}( \mu x_1. \const{A}( {\Varec{x}} ) ) )$.
$\Box$
\end{example}
...
%and $\muvar( w_i ) = {\Varec{t_i}}$ for $i = 1, \ldots, n$,
%\item
...
%Each $w_i$ is either ${\Varec{x}}$,
%a variable ${\Varec{y}}$ where $\ec{y}$ does not contain a constructor,
%or a term of the form $\mu {\Varec{y}}. \const{C}_j( \vec u )$.
While interpreting $w_i$ in $\Val \ec{x}$,
all free occurrences of ${\Varec{x}}$ in $\Val \ec{x} \interp{w_i}$ are replaced by $\Val \ec{x}$. \rem{better formalize.}
...
%Say that $\muvar( w_i ) = {\Varec{y_i}}$.
%Due to our construction of $\Val^\ast$ and since \rn{Inject} does not apply to $\Fc$, we have $\ec{u_i} = \ec{y_i}$,
...
%By our construction of $\Val^\ast$ and since \rn{Inject} and \rn{Clash} do not apply to $\Fc$, 
%$\Val^\ast \ec{ u }$ must be of the form $t_u = \mu {\Varec{u}}. C_i( u_1, \ldots, u_n )$,
%where $\muvar( u_1 ) = {\Varec{s^1_i(u)}}$, $\ldots$, $\muvar(u_n) = {\Varec{s^n_i( u )}}$,
...
$\const{C}_i( \vec u )$, then we immediately infer $s^j_i( t ) \teq u_j$, whereas the calculus we presented would apply \rn{Split} and \rn{Inject} before inferring this equality.  
...
Phase 1 (along with the optimization for collapsing selectors) and the \rn{Singleton} rule are performed eagerly, 
that is for partial satisfying assignments $M$, while phase~2 and the \rn{Split} rule from phase~3 are performed only for complete satisfying assignments $M$, and in that order.
