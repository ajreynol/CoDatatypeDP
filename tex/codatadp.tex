...
Let $T$ be the set of terms in $\tEc$ whose types are finite datatype.
...
and let $\tcEc$ be the set of subterms of $\Ec \mathrel\cup \{ \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) ) \mid t : \tau \in \SSS \mathrel\cup \SSSS \ast, \const C_i \in \Ctr^\tau \}$.
...
Thus, the depth of a branch in a derivation tree with root node $\Ec$ is at most $\left| \tcEc \right|^2$,
...
then by the induction hypothesis $\Ec \mathrel\cup t \teq \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) )$ is unsatisfiable
for each $\const C_i \in \Ctr^\tau$.
Since by exhaustiveness, all models of $\thD$ entail (exactly) one equality $t \teq \const C_i( \const s^1_i( t ), \ldots, \const s^{a_i}_i( t ) )$,
we know $\Ec$ is $\thD$-unsatisfiable.
Otherwise, the child of $\Ec$ is a closed derivation tree
whose root node is $\Ec \mathrel\cup t_1 \teq t_2$ obtained by applying one of the rules \rn{Refl}, \rn{Sym}, \rn{Trans}, \rn{Cong}, \rn{Inject}, \rn{Unique}, or \rn{Singleton}.
...
By the induction hypothesis, we have $\Ec \mathrel\cup t_1 \teq t_2$ is $\thD$-unsatisfiable,
...
$\thD$-satisfiable.
To do so, we exhibit a particular model $\M$ of $\thD$ that satisfies $\Ec$.
...
\textstyle\begin{cases}
    \MU x.\; \const{C}( t_1\interp{ \vec u}^{M \mathrel\cup \{ x \}}) & \text{if }x \not\in M, x \mapsto \MU x.\; \const{C}( \vec u ) \in t_1  \\[-\jot]
    x & \text{otherwise}
\end{cases} \\
t_1\interp{\MU x.\; \const{C}( \vec u )}^M & = &
\textstyle\begin{cases}
    \MU x.\; \const{C}( t_1\interp{ \vec u }^{M \mathrel\cup \{ x \}} ) & \text{if }x \not\in M \\[-\jot]
    x & \text{otherwise}
\end{cases}
...
The term $t_1$ is \emph{normal} if it does not contain a strict, bisimilar subterm
and all of its strict subterms are also normal.
For example, the $\mu$-term $t_1$ of the form $\MU x.\; \const{C}( \MU y.\; \const{C}( y ) )$ is not normal,
...
For any $\mu$-term $t$ of the form $\MU x.\; \const C( \vec u )$, we may recursively construct a normal $\mu$-term $\nf{t}$
...
For example, say we assign $\MU {\Varec{x}}.\; \const{B}( {\Varec{x}} )$ for ${\Varec{x}}$.
...
If $k \neq i$, then we interpret $\const s^j_k( \M( u ) )$ as the value in $\M( \tau )$ that is $\alpha$-equivalent with $\Val^\ast \ec{ t }$.
...
$\const{C}_i( \vec u )$, then we immediately infer $\const s^j_i( t ) \teq u_j$, whereas the calculus we presented would apply \rn{Split} and \rn{Inject} before inferring this equality.
...
$(\const{C}_1( \const s^1_1( t ), \ldots, \const s^{a_1}_1( t ) ) \vee \ldots \vee t \teq \const{C}_n( \const s^1_n( t ), \ldots, \const s^{a_n}_n( t ) ))$
to $F$.
