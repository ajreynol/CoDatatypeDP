...
%\begin{lemma}
%If $t_1$ and $u$ are normal, and if $u \not\vsimv{x} t_1\interp{t_2}$ for all $t_2 \in \mathcal{T}( t_1 )$,
%then $t_1 \{ x \mapsto u \}$ is normal.
%\end{lemma}
%\begin{proof}
%\rem{TODO}
%\end{proof}
...
%equivalence class are interpreted as values,
%the interpretation of an equivalence class is not sensitive to the context in which it is interpreted,
%and that each equivalence classes has a distinct interpretation.
...
For the latter, let $\M( x )$ be the value in $\M$ that is a renaming of $\Val^\ast \ec{x}$ for each variable $x$ in $\mathcal{T}(\Fc)$.
...
then $\M( t )$ is a renaming of $\nf{(\mu x. \const{C}( \M( u_1 ), \ldots, \M( u_n ) ) )}$ for fresh $x$,
which by the induction hypothesis is bisimilar to $\nf{(\mu x. \const{C}( \Val^\ast \ec{ u_1 }, \ldots, \Val^\ast \ec{ u_n } ) )}$, call this term $t_u$.
Now, since \rn{Unify$_2$} does not apply to $\Fc$,
then $\Val^\ast \ec{t}$ must be a term $t_w$ of the form $\mu v_{\ec{t}}. \const{C}( w_1, \ldots, w_n )$,
that is, an application of the same construction $\const{C}$.
For each $i = 1, \ldots, n$,
say that $\muvar( w_i ) = v_{\ec{y_i}}$.
Due to Lemma~\ref{lem:model-completion}.2, $t_w\interp{ w_i } = \Val^\ast \ec{ y_i }$.
Due to our construction of $\Val^\ast$ and since \rn{Unify$_1$} does not apply to $\Fc$, we have $\ec{u_i} = \ec{y_i}$.

If $t$ is a selector term $s^j_k( u )$,
by the induction hypothesis, $\M( u ) \vsim \Val^\ast \ec{ u }$. 
since \rn{Split} does not apply, 
$\ec{u}$ must contain a term of the form $C_i( s^1_i( u ), \ldots, s^{a_i}_i( u ) )$ for some $i$.


%we show that $w_i$ is bisimilar to the $i^{th}$ argument of
%$\nf{(\mu x. \const{C}( \Val^\ast \ec{ u_1 }, \ldots, \Val^\ast \ec{ u_n } ) )}$.
%In the case that $\mu
%Since $t_c$ is normal
...
%If $\tau \in \Data \cup \Codata$, we show $\M \models t_1 \teq t_2$ by showing 
%We show by induction on the term size of $t$ that $\M( t ) = \Val^\ast \ec{t}$.
%If $t$ is a variable, then $\M( t ) = \Val^\ast \ec{ t } = \Val^\ast \ec{ x }$.
%In the case where 
%where say $\muvar( v_i ) = v_{\ec{y_i}}$ for $i = 1, \ldots, n$.
%We have $\M( \const{C}( \vec u ) ) = \nf{(\mu y. \const{C}( \M( \vec u ) ))}$.
