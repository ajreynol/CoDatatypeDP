...
and in the third part we (when necessary) split on the constructor type for various terms in $\tEc$.
...
or a variable $x$.
We assume all $\mu$-terms are \emph{well-typed}, which we defined recursively:
...
(in other words, there are initially no constraints on the values for terms).
...
our construction of $\Val$ is terminating.

\begin{example}
Say that $\Ec$ contains equivalence classes $\ec{x}$ and $\ec{y}$ where $\const{C}(z,x) \in \ec{y}$ and $\const{C}(w,y) \in \ec{x}$.
...
\hline
\end{array}\]
In other words, the (class of) values for $v_{\ec{x}}$ in models of $\Ec$ 
is $\const{C}( v_{\ec{w}}, \const{C}( v_{\ec{z}}, \ldots ))$, 
and the class of values for $v_{\ec{y}}$ is $\const{C}( v_{\ec{z}}, \const{C}( v_{\ec{w}}, \ldots ))$,
where $v_{\ec{z}}$ and $v_{\ec{w}}$ are free variables.
$\Box$
\end{example}

Given $\Val$, the rules for cyclicity and bisimilar can be stated as follows, and are shown in Figure~\ref{fig:ab-rules}.
For cyclicity, if there is an equivalence class $\ec{t}$ of datatype type whose possible values $\mu x. t'$ are cyclic 
...
of co-datatype type whose possible values $\Val \ec{t_1}$ and $\Val \ec{t_2}$ are equivalent modulo renaming of $\mu$-bound variables,
...
Our construction of $\Val$ gives us:
\[\begin{array}{l@{\hspace{.5em}}l@{\hspace{.5em}}l}
\Val \ec{x} & = & 
...
and thus our calculus may infer the equality $x \teq \const{C}(y,x)$.
\qed
\end{example}
...
Recall that the $\mu$-notation can be used to represent (possibly cyclic) values taken from an initial algebra, \rem{elaborate?} 
e.g. $\mu x. \const{C}( \const{0}, x )$ represents the infinite sequence $\const{C}( \const{0}, \const{C}( \const{0}, \ldots ))$.
In this setting, a value can be viewed as a term-like structure where all subterms 
(which we will refer to as \emph{subvalues}) are either 
$\mu x. \const{C}( \vec v )$ for some constructor $\const{C}$ and subvalues $\vec v$
\footnote{We assume all values are well-typed. 
Here, the type of $\mu x. \const{C}( \vec v )$ and $x$ is the return type of $\const{C}$.}, or a variable $x$.
We will refer to values of this form as \emph{$\mu$-values}.
We say a $\mu$-value is \emph{closed} if it contains no free variables.
For convienience, we will represent the values of both datatypes and co-datatypes as closed $\mu$-values,
where the case of datatypes adds the restriction that none of its subvalues are variables.

We introduce the following notions concerning $\mu$-values.
First, we write $v \vsim u$ if $\mu$-values $v$ and $u$ are equivalent modulo renaming of $\mu$-bound variables.
...
In our construction, the rule \rn{Ind} is first applied exhaustively.
Since the height of any value mapped to by $\Val$ is bounded by the number of equivalence classes of $\Fc$,
the rule \rn{Ind} can be applied only a finite number of times.
Second, \rn{Base} is applied exhaustively.
Since \rn{Base} removes one variable from the set $FV( \Val )$ and does not add any additional variables to $FV( \Val )$, 
the rule \rn{Base} can only be applied a finite number of times and does not trigger any new applications of \rn{Ind}.

We first argue that $\Val$ contains only ground values.
First, since \rn{Split} does not apply to $\Fc$,
all equivalence classes of finite type contain constructor terms.
Thus, after applying \rn{Ind} exhaustively, the free variables of $\Val$ all have infinite type.
