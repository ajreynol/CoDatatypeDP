...
the equivalence class of $t$ must contain an application of one of the constructors of $\tau$ (where $\Ctr^\tau = \{ C_1, \ldots C_n \}$).
This is enforced in the third phase by the \rn{Split} rule.
The \rn{Singleton} rule handles the degenerate case where two terms from $\tEc$
have a corecursive singleton type, in which case they must be equivalent.
...
\textstyle\begin{cases}
    \mu x.\; \const{C}( t_1\interp{ \vec u}^{M \cup \{ x \}}) & \text{if }x \not\in M, x \mapsto \mu x. \const{C}( \vec u ) \in t_1  \\[-\jot]
    x & \text{otherwise}
\end{cases} \\
...
We construct $\Val^\ast$ by exhaustively applying the following update to $\nf{(\Val)}$:
...
to ${\Varec{x}}$.
Legal substitutions for ${\Varec{x}}$ may cause the range of $\Val$ to contain non-normal terms.
...
\item
All subterms $t$ of $\Val^\ast \ec{y}$ where $\muvar(t) = {\Varec{x}}$ are such that
...
were uniquely of the form $\{ {\Varec{z}} \mapsto \const{C}_i( {\Varec{t_1}}, \ldots, {\Varec{t_n}} ) \}$
for $\ec{z}$ that contain a constructor $\const{C}_i( t_1, \ldots, t_n )$.
