...
The metavariables $\delta,\:\varepsilon,\:\tau$ range over types, with $\delta$ reserved
...
A related semantic view of datatypes is as an initial algebra. Codatatypes are
then defined dually as a final coalgebra. The datatypes are generated by their
...
emphasizes the commonality between datatypes and codatatypes, while
sacrificing a theoretically fruitful notion of duality.
...
semantic notion is more fruitful and is the one that interests us.

Furthermore, nothing forbids specifications with no (co)recursion, such as
\[\begin{aligned}[t]
...
\end{aligned}
\]
Semantically, it makes no difference whether the types are introduced as
datatypes or as codatatypes.
%Without loss of generality, we consider that these types are datatypes
%instead of codatatypes.

Finally, some codatatypes are so degenerate that they have infinite values
even though they are finite. The simplest example is
\[\keyw{datatype}\;\, \ty{a} \,=\, \const{A}(\ty{a})\]
...
\[\begin{aligned}[t]
      \keyw{codatatype}\;\, \ty{b} & \,=\, \const{B}(\ty{b},\: \ty{c},\: \ty{b},\: \ty{unit}) \\[-\jot]
             \keyw{and}\;\, \ty{c} & \,=\, \const{C}(\ty{unit},\: \ty{c},\: \ty{b},\: \ty{c})
\end{aligned}
\]
where \ty{unit} is a datatype with the single constructor $\const{Unity} :
\ty{unit}$. We call such types \emph{corecursive singletons}.
For the decision procedure, it will be important to detect those finite
codatatypes. Sometimes, a type might be a corecursive singleton only in some
models. For example, if \ty{unit} is an uninterpreted type, \ty{b} and \ty{c}
will be singleton types precisely in the models that interpret \ty{unit} as a
singleton.

% In other words, all finite corecursive codatatypes are corecursive singletons.

\begin{lemma}
If the domain interpreting a corecursive codatatype is finite, it must be a
singleton. Furthermore, such a type has necessarly a single constructor and
all its arguments have types that are interpreted by singletons.
\end{lemma}

\begin{proof}
  * if two (either directly or indirectly) corecursive constructors
    $\const{C}$ and $\const{D}$, it is
    possible to encode infinitely many alternation patterns,
    e.g. $\const{C}(\const{D}(\const{C}(\const{C}(\ldots))))$;
    hence no finite models

  * if one corecursive and one noncorecursive, then possibility to create
    objects of arbitrary depths, e.g. $\const{C}(\ldots(\const{C}(\const{D}))\ldots)$,
    and hence no finite models

  * leaves us with the case of a single corecursive constructor

  * if any of the noncorecursive arguments has cardinality greater than one,
    then it is possible to encode alternation patterns using it,
    e.g. $\const{C}(0,\: \const{C}(1,\: \const{C}(0,\: \const{C}(0,\: \ldots))))$.

  * otherwise, by the coinduction principle, there is only one value that can
    exist
