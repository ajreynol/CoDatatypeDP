...
For example, the $\mu$-term $t = \MU x.\; \const{C}( \MU y.\; \const{C}( y ) )$ is subnormal,
...
is syntactically equivalent to $t\interp{x}^\emptyset = t$ for the renaming $\{ x \mapsto y, y \mapsto x \}$.
...
to ${\Varec{x}}$.
Legal substitutions for $v_{\ec{x}}$ may cause the range of $\Val$ to contain subnormal terms.
