...
since $\MU y.\; \const{C}( y )$ is a self-similar subterm of $t$.
...
which are not $\alpha$-equivalent by statement 3, contradicting the fact that $\MU {\Varec{y}}.\; \const{C}( u_1, \ldots, u_n )$ is a self-similar subterm of $\Val \ec{x}$.
...
%where $\interp{\Val^\AST \ec{t}}{w_i} = \Val^\AST \ec{u_i}$ for $i \in [n]$.
...
Thus $\interp{t_u}{u_\iPrime} \vsim \interp{\Val^\AST \ec{t}}{ w_i }$. % for each $i \in [n]$.
