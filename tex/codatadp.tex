...
denoting nondeterministic branching.
...
For unification, additional equalities are inferred based on the injectivity of constructors by \rn{Decomp},
...
\rn{Bisimilar}
\end{tabular}
\caption{\,Directional rules for acyclicity and bisimilarity%.
}
\label{fig:ab-rules}
\end{figure}
...
When this is the case, it is easy to see that $\Ec$ induces an equivalence relation over $\tEc$ such that two terms $t$ and $u$ are equivalent if and only if $t \teq u \in \Ec$.
