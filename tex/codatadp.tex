...
This will simplify the presentation later. % of the decision procedure.
...
is a smallest model. For example,
...
witnesses $R$ that are required to be bisimulations:
%
\[
\begin{aligned}[t]
...
these give rise to spurious equalities between unrelated terms---e.g.,
$\const{s}_{11}(\const{C}_2) \teq \const{s}_{11}(\const{C}_3)$. This could be
corrected, but the added complexity seems to suggest that selectors are better
...
\ty{unit}$. We call such types \emph{corecursive singletons}. For the decision
procedure, it will be important to detect these types. A type may also be a
corecursive singleton only in some models. For example, if \ty{unit} is an
uninterpreted type, \ty{b} and \ty{c} will be singleton types precisely in the
models that interpret \ty{unit} as a singleton. Fortunately, given some
cardinalities for the ordinary types, it is easy to characterize this
degenerate case:
...
singleton. Furthermore, such a codatatype has necessarly a single constructor,
and all its arguments have types that are interpreted by singletons.
\end{lemma}
