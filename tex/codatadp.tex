...
\inferrule{
  ...
  c \in \M
  \quad
  c \not\sim t \sigma \text{ for all } \sigma, t \in \vrange(\Val)
}{
  \Val := \Val \{ v_{\ec{x}} \mapsto c \}
}
\)

%We write $\mathcal{T}(\Val)$ to denote the smallest set containing $v\interp{u}$ for each subterm $u$ of $v$ in the range of $\Val$.
...
To argue this is the case, notice that either $\tau$ is a recursive type, or contains an infinite subfield of type $\upsilon$.
In the former case, we choose a value $c_1$ whose depth is greater than any term in $\vrange(\Val)$.
In the latter case, we choose a value $c_1$ containing a subterm of type $\upsilon$ not yet occurring in $\Val$.
...
We now argue the following invariant is maintained by applications of \rn{Complete}:% for all $\ec{x}, \ec{y}$:
\begin{eqnarray}
%\label{eqn:m-normal} 
%\Val \text{ maps to normal $\mu$-terms only } \\
...
\label{eqn:m-bij} 
\Val \ec{x} \sim \Val \ec{y} \text{ if and only if } \ec{x} = \ec{y}
\end{eqnarray}
Initially, since \rn{Bisimilar} does not apply to $\Fc$,
we have that $\Val \ec{x} \not\vsim \Val \ec{y}$ for all distinct $\ec{x}$ and $\ec{y}$,
and thus~\ref{eqn:m-bij} holds.
...
Now, we assume $\Val$ satisfies~\ref{eqn:m-bij},
and show that $\Val \sigma$ does as well, where $\sigma := \{ v_{\ec{x}} \mapsto c_1 \}$ is a substitution applied by \rn{Complete}.
By contradiction, assume that $\Val \sigma \ec{y} \sim \Val \sigma \ec{z}$ for some distinct $\ec{y}$ and $\ec{z}$.
Then it must have been the case that $\Val \ec{y}$ and $\Val \ec{z}$ contain a position $p$
where, w.l.o.g., say $\Val \ec{y} \mid_p$ is $v_{\ec{x}}$ and $\Val \ec{z} \mid_p$ is a closed $\mu$-term $c_2$ where $c_1 \sim c_2$.
%Since $\Val$ satisfies invariant~\ref{eqn:m-bij},
%we have that $\Val \ec{w} \sim c_1$ for some $\ec{w}$, 
However, this contradicts the premise of \rn{Complete} that $c_1$ is not bisimilar to any $c_2 \in \mathcal{T}( \Val )$.
Thus, by induction on the number of applications of \rn{Complete}, statement~\ref{eqn:m-bij} holds for $\Val^\ast$.
...
Now, we let $\M( x )$ be the value in $\M$ that is a renaming of$\Val^\ast \ec{x}$ for each variable $x$ in $\mathcal{T}(\Fc)$,
which are guarenteed to be distinct values due to statement~\ref{eqn:m-bij} and our construction of $\M$.
