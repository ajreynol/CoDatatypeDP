...
the \emph{interpretation of $u$ in $t$} is the $\mu$-term $\interp{t}{u}^\emptyset$ as returned by the following recursive procedure,
...
\textstyle\begin{cases}
    \MU x.\; \const{C}( \interp{t}{ \vec u}^{M \mathrel\cup \{ x \}}) & \text{if }x \not\in M\text{~and $x$ is
    bound by $\MU x.\;
\const{C}( \vec u )$}  \\[-\jot]
    x & \text{otherwise}
\end{cases} \\
\interp{t}{\MU x.\; \const{C}( \vec u )}^M & = &
\textstyle\begin{cases}
    \MU x.\; \const{C}( \interp{t}{ \vec u }^{M \mathrel\cup \{ x \}} ) & \text{if }x \not\in M \\[-\jot]
    x & \text{otherwise}
\end{cases}
\end{array}\]
We write $\interp{t}{u}$ to abbreviate $\interp{t}{u}^\emptyset$.
...
$\MU y.\; \const{C}( u_1, \ldots, u_n )$ and $\interp{t}{ t_k } \aequiv \interp{t}{ u_k }$ for $k = 1,\ldots, n$.
...
that is,
$\interp{t}{\MU y.\; \const{C}( y )}^\emptyset =$
...
noting that
$\interp{t}{\MU y.\; \const{C}( x )}^\emptyset =
...
is $\alpha$-equivalent to $\interp{t}{x}^\emptyset = t$.
...
where $\interp{\Val \ec{x}}{ t_i } \aequiv \interp{\Val \ec{x} }{ u_i }$ for $i = 1, \ldots, n$.
...
%where $\interp{\Val^\AST \ec{t}}{w_i} = \Val^\AST \ec{u_i}$ for $i = 1, \ldots, n$.
...
Thus $\interp{t_u}{u_\iPrime} \vsim \interp{\Val^\AST \ec{t}}{ w_i }$. % for each $i = 1, \ldots, n$.
...
%where $\interp{\Val^\AST \ec{u} }{ w_i } = \Val^\AST \ec{ \const s^i_{\negvthinspace\jPrime}( u )}$ for $i = 1, \ldots, n$. 
Since \rn{Inject} and \rn{Clash} do not apply to $\Fc$, 
