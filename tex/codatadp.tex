...
If $u$ is a subterm of $v$, then the \emph{interpretation of $u$ in $v$} is the term $v\interp{u}^\emptyset$ returned by the following recursive procedure,
...
\end{array}\]
Given a $\mu$-term $v$ of the form $\mu x. C( \vec w )$, 
a subterm $u$ of $v$ is a \emph{bisimilar subterm} if and only if $v\interp{u}^\emptyset \vsim v$.
We say $v$ is \emph{normal} if and only if all of its bisimilar subterms are $x$,
and all of its strict subterms that do not contain $v$ are also normal.
...
Let $\Val^\ast$ be the result of applying the above rule to completion on $\Val$.
...
If $\Val \ec{x} = v_{\ec{x}}$, then we may apply the above rule for the (single) value $c$ of type $\tau$, 
