...
\\
  ...
  \Ec := \Ec,\: t \teq \const{C}_1\bigl(\const s^1_1( t ), \ldots,\const s^{n_{1\!}}_1( t ) \bigr) %\ROR
...
we may no longer assume that $\ty{u}$ is of infinite cardinality.
...
For example, consider the codatatype $\keyw{codatatype}~\ty{c} = \const{C}(\ty{u},\: \ty{c})$, where $\ty{u}$ is an uninterpreted type.
This codatatype has cardinality one when $\ty{u}$ has cardinality one.
If there exist two equivalence classes $\ec{s\typ{c}}$ and $\ec{t\typ{c}}$,
our implementation adds the clause %of the form
$\exists x\;y\typ{\ty{u}}.\;\, x \not\teq y \mathrel\vee s \teq t$ to $F$.
In other words, either the cardinality of $\ty{u}$ is greater than one or $s$ is equal to $t$.
