...
\]
%
The rule chooses an unassigned variable in $\Val$ and assigns it a value that is
not $\alpha$-equivalent to one occurring in
...
lose its modelhood.%
\rem{Improve terminology.}
% "be inconsistent." Surely, models can't be "inconsistent".
...
On the other hand, if the value assigned to ${\Varec{x}}$ is not $\alpha$-equivalent to any term in $\mathcal{T}^{\smash{\M}}_{{\Varec{x}}}( \Val )$,
...
Intuitively, this lemma states three properties of $\ValC$ which ensure a normal model $\M$ can be trivially constructed for $\Ec$.
Part~\PART{1} states that the values in the range of $\ValC$ are $\alpha$-equivalent to a value in normal model.
In particular, this means they are closed, normal, and acyclic when required.
Part~\PART{2} states that the interpretation of all subterms in the range of $\ValC$ depends on its associated variable only.
In other words, the interpretation of a subterm $t$ where $\muvar(t) = {\Varec{x}}$ is equal to $\ValC \ec{x}$, independent of the context in which it is interpreted.
Part~\PART{3} states that $\ValC$ is injective, which in the following ensures that a distinct values can be associated with each equivalence class in a corresponding model.
...
Our implementation uses several optimizations not reflected in the presentation of the calculus, which closely follow the approach outlined by Barrett et al.\ \cite{barrett-et-al-2007}.
