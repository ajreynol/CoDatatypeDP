%%% TODO: Clarify that \mu-terms appear in generated models
...
%\newcommand\XXXL{DPLL}
\newcommand\XXXL{CDCL} %% TODO: Convince Andy this is right
...
\newcommand{\interp}[2]{\langle \smash{#2} \rangle_{#1}}
...
\noindent
of infinite sequences, or streams, over an element type $\tau$ is allowed\begin{rep}; the
...
induction in both types of provers \cite{kersani-peltier-2013,wand-weidenbach-201x,reynolds-kuncak-2015}.
%
Datatypes are explicitly supported by the higher-order model finder Refute
...
inspired by the $\mu$-calculus \cite[etc.]{kozen-1983,endrullis-et-al-2011}.
...
disequalities between $\Sig$-terms, taken as a conjunction. We
...
Finally, since the constraints in $\Ec$ are ground, they cannot entail an upper
bound on the cardinality of any uninterpreted type $\tau \in \Nondata$; thus,
...
% \betweenantes
 ...
 \Val \ec{t\typ{\delta}} \aequiv \Val \ec{u\typ{\delta}}
}{
 \Ec := \Ec,\: t \teq u
}
\)
...
(as required by the precondition $x \in \FV( u )$),
...
then $t \teq u$. This precondition may seem too restrictive, since
...
%are equivalent.
%
At this point, phase~1 is activated again to refine the equivalence classes,
yielding
$\{ x,\, \const{S}(x) \}$ and
...
%for the renaming
%$\{ {\Varec{y}} \mapsto {\VAREC{\const{S}(y)}}, {\VAREC{\const{S}(y)}} \mapsto {\Varec{y}} \}$,
the \rn{Unique} rule can finally derive $x \teq y$.
\xend
\end{examplex}
...
constraint on the codatatype in \rn{Single} can be evaluated statically
...
This is a finite set for each $t$ because the values of the type of $t$ are of finite size\rem{Clarify}.
...
whose root node $\Ec \mathrel\cup t \teq u$ is obtained by applying one of the rules \rn{Refl}, \rn{Sym}, \rn{Trans}, \rn{Cong}, \rn{Inject}, \rn{Unique}, or \rn{Single}.
...
For \rn{Unique}, our construction of $\Val$ indicates that the values of $t$ and $u$ are equivalent in all models of $\Ec$.
...
\rem{TODO: Normal model vs. actual model.}
\rem{TODO: Model or interpretation? Grep elsewhere.}
...
if $\M( t )$ is of the form $\MU x.\; \const{C}_{\negvthinspace j}( \vec u )$, 
then $\M\negvthinspace\bigl( \const s^k_{\negvthinspace j}( t ) \bigr)$ is the value
in $\M( \tau )$ that is $\alpha$-equivalent to
$\interp{\M( t )}{u_k}$.
\end{enumerate}
\end{definition}
...
\rem{Variables are really part of the model then?}
...
we write $\mathcal{T}^{\smash{\M}}_x( \Val )$ to denote the set consisting of all values
$v \in \M( \tau )$
such that $v \vsimv{x} \interp{t}{u}$ for some term $t$ with subterm $u$ occurring in the range of $\Val$.
\rem{Intuition?}

The \emph{completion} $\ValC$ of $\Val$ for a normal model $\M$
assigns values from $\M$ to unassigned variables in the domain of $\Val$.
...
\inferrule{
  ...
  v \in \M( \tau )
  \betweenantes
  v \not\in \mathcal{T}^{\smash{\M}}_{\Varec{x}}(\Val)
}{
...
value---one that does not occur in $\mathcal{T}^{\smash{\M}}_{\Varec{x}}(\Val)$
modulo $\alpha$-equivalence.
\rem{The ``not alpha equivalent'' bit is expressed differently in the rule,
with $\notin$. This looks fishy.}
\rem{What guarantees that such a value exists? (Answer: infinitely many values
of codatatypes, of which only finitely many are in
$\mathcal{T}^{\smash{\M}}_{\Varec{x}}(\Val)$?)}
...
the range of $\Val$ after each rule application.\rem{clumsy}
...
Modulo $\alpha$-equivalence, the set \smash{$\mathcal{T}^{\smash{\M}}_{{\Varec{x}}}( \Val )$} is
\[%\begin{array}{c}
\{
\MU x.\; \const{D}( x ),\;
...
%\end{array}
\]
Consider a normal interpretation $\mathbf{J}$ \rem{Appropriate symbol?} that interprets variables in $\Ec$ based on the mapping $\Val$:
$x$ is interpreted as $\Val \ec{x}$, $y$ as $\Val \ec{y}$, and so on.
Assigning a value for $\Val \ec{x}$ that is $\alpha$-equivalent to a value in $\mathcal{T}^{\smash{\M}}_{{\Varec{x}}}( \Val )$
may cause values in the range of $\Val$ to become $\alpha$-equivalent,
which in turn may cause $\Ec$ to be falsified by $\mathbf{J}$.
...
On the other hand, if the value assigned to ${\Varec{x}}$ is fresh, %not $\alpha$-equivalent to any term in $\mathcal{T}^{\smash{\M}}_{{\Varec{x}}}( \Val )$,
...
\begin{enumerate}
\item[\rm (1)] $\ValC \ec{x\typ{\tau}}$ is $\alpha$-equivalent to a value in $\M( \tau )$.
\item[\rm (2)]
...
and thus there are no terms in $\mathcal{T}^{\smash{\M}}_{\Varec{y}}(\ValC)$ of type $\tau$.
...
since there are only a finite number of closed terms in $\mathcal{T}^{\smash{\M}}_{\Varec{y}}(\ValC)$,
and thus our model completion can assign a value not occurring in $\mathcal{T}^{\smash{\M}}_{\Varec{y}}(\ValC)$ to ${\Varec{y}}$.
...
where $\interp{\Val \ec{x}}{ t_i } \aequiv \interp{\Val \ec{x} }{ u_i }$ for some $i$. % \in [n]$.
Due to our construction of $\Val$, $\ec{y}$ contains a constructor $\const{C}( w_1, \ldots, w_n )$ and $\muvar( u_i ) = w_i$ for some $i$. % \in [n]$.
...
${\Varec{x}} \in FV( \Val_1 )$,
and $\MU {\Varec{x}}.\; t$ is not $\alpha$-equivalent to a term in $\mathcal{T}^{\smash{\M}}_{\Varec{x}}(\Val_1)$.
To show 2,
...
there is a closed $\mu$-term $v \in \mathcal{T}^{\smash{\M}}_{\Varec{x}}( \Val_1 )$
...
interpretation $\mathbf{J}$ can be constructed that satisfies $\Ec$. Property~\PART{1} states that the
...
We consider a normal model $\M$ \rem{Change to interpretation, with proper symbol.}
...
which by Lemma~\ref{lem:model-completion}\PART{1} is guaranteed to exist.
...
of the form $\MU {\Varec{t}}.\; \const{C}( w_1, \ldots, w_n )$
where for each $i \in \{1,\ldots, n\}$, % \in [n]$,
$\muvar(w_i) = {\Varec{u_i}}$.  Thus by Lemma~\ref{lem:model-completion}\PART{2},
...
Due to the induction hypothesis and Lemma~\ref{lem:model-completion}\PART{3}, $\M( u ) \neq \M( u' )$ for all such $u$, $u'$.
...
formulas were described at a high level of abstraction. This section describes
the implementation within the SMT solver CVC4.

\paragraph{Integration into {\relax\XXXL($T$)}.}
The calculus is implemented as a \emph{theory solver} of CVC4, that is, a
specialized solver for determining the satisfiability of conjunctions of
constraints for its theory.
Given a theory $T = T_1 \mathrel\cup \cdots \mathrel\cup T_n$ and an input set of
clauses $F$ in CNF, the \XXXL($T$) procedure
incrementally builds partial assignments from the atomic formulas of $F$ to
truth values such that no clause in $F$ is falsified. We may interpret such a
partial assignment as a set of literals where $a \in M$ for all atoms $a$ that
it assigned the value $\top$, and $\neg\; a \in M$ for all atoms that it assigns
the value $\bot$. Then, at a high level, by the Nelson--Oppen combination
framework (its variant in CVC4 is described in~\cite{jovanovic2011sharing}), 
...
containing symbols not belonging to $T_i$ are replaced by fresh variables of the
same type; (2) additional equalities and disequalities between variables of
...
Assume $\Ec$ is initially the set $M_i$ described above. For each equality $t \teq s$ added
to $\Ec$, we associate a set of equalities from $M_i$ that together entail $t
\teq s$, which we call its \emph{explanation}.
Similarly, each $\Val \ec{x}$ is also associated an explanation---that is, a set
...
$\thD$-unsatisfiable. For \rn{Split}, we add a clause %of the form
$t \teq \const{C}_1\bigl( \const s^1_1( t ), \ldots, \const s^{n_1}_1( t ) \bigr)
...
solver and not by the theory solver. All other rules add equalities to the
...
Prior to constructing a model for $F$, the theory solver does not explicitly construct $\mu$-terms nor the map $\Val$.
Instead, $\Val$ is computed implicitly by traversing the equivalence classes of $\Ec$ during phase~2.
To recognize when the precondition of \rn{Acyclic} holds,
the procedure considers each datatype equivalence class $\ec{t}$ containing a constructor $\const{C}( t_1, \ldots, t_n )$.
It visits $\ec{t_1}, \ldots, \ec{t_n}$ and all arguments of constructors in these equivalence classes in the same way.
If while doing so it returns to $\ec{t}$, then it deduces that \rn{Acyclic} applies to $t$.
To recognize when the precondition of \rn{Unique} holds,
the procedure considers the set $S$ of all codatatype equivalence classes.
It simultaneously visits the equivalence classes of arguments of constructor terms in each equivalence class in $S$,
while partitioning $S$ into $S_1, \ldots, S_n$ based on the top-most symbol of constructor terms in these equivalence classes
...
If after doing so, the resulting partition contains a set $S_i$ containing distinct terms $u$ and $v$, then it deduces that \rn{Unique} applies to $u$ and $v$.

%\rem{TODO: * sharing is caring (let's cite it for a change)}
\rem{TODO: ``theory combination more complicated (and Theorem Completeness too weak)''?}
%\rem{TODO: representation of $\mu$-terms?}

\paragraph{Extension to Quantified Formulas.}
While our calculus is a decision procedure for ground constraints in the theory of datatypes and codatatypes,
...
Many SMT solvers, such as CVC4, support reasoning about quantified formulas using incomplete instantiation-based methods~\cite{MouraBjoerner07,ReynoldsTinelliMoura14}.
...
Namely, since quantified formulas may entail cardinality constraints on uninterpreted types $U$,
we may no longer assume that $U$ is of infinite cardinality.
When assuming that a finite type has infinite cardinality, the calculus presented in this paper is \emph{incomplete},
since the preconditions on \rn{Split} and \rn{Single} may not apply when they indeed are applicable.
...
Regardless, we may be interested in increasing the precision of the solver by doing the following.
First, we may apply \rn{Split} to terms of datatype types whose cardinality depends on the finiteness of uninterpreted types.
Second, we may conditionally apply \rn{Single} to terms of codatatype types that may have cardinality one.
For instance, consider the codatatype $\keyw{codatatype}~\ty{c} = \const{C}(\ty{U},\: \ty{c})$, where $U$ is an uninterpreted type.
This codatatype has cardinality one when $\ty{U}$ has cardinality one.
If there exists two equivalence classes $\ec{t\typ{c}}$ and $\ec{u\typ{c}}$,
our implementation adds a clause of the form $( \exists x\,y.\, x \not\teq y \vee t \teq u )$ to $F$,
where $x$ and $y$ are variables of type $U$.
In other words, either the cardinality of $U$ is greater than one, or $t$ must be equal to $u$.

    
\paragraph{Optimizations.}
Our implementation uses several optimizations which
...
We only briefly mention these optimizations, since
each applies to datatypes as well as codatatypes, and most have been presented
thoroughly in Barrett et al.
...
\begin{rep}  % lower priority to mention this
For the sake of reducing the number of unique constraints considered by the calculus, we
...
included all the theory files from the Isabelle distribution (Distro) and the \emph{Archive
...
experiments. The experimental data is publicly available.%
\footnote{\url{http://TODO/FIXME.}}
...
tautologies. Z3 is included for comparison when this is possible.
...
\begin{center}\begin{tabular}{@{\;}l@{\kern1.5em}cc@{\kern.5em}cc@{\kern.5em}cc@{\kern1.3em}cc@{\;}}
  & \multicolumn{2}{c}{Distro} & \multicolumn{2}{c}{AFP} & \multicolumn{2}{c}{\gandl} & \multicolumn{2}{c}{Overall} \\
  & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4}
\MIDRULE
No (co)datatypes
  & 209 & 221 & 777 & 775 & \phantom{0}51 & \phantom{0}52 & 1037 & 1048 \\%[\jot]
%Datatype simplication only
%  &  \\
Datatypes w/o \rn{Acyclic}
  & & 227 & & 780 & & \phantom{0}52 & & 1059 \\
Full datatypes
  & 213 & 227 & 791 & 786 & \phantom{0}51 & \phantom{0}52 & 1055 & 1065 \\%[\jot]
%Codatatype simplication only
%  &  \\
Codatatypes w/o \rn{Unique}
  & & 222 & & 804 & & \phantom{0}56 & & 1082 \\
Full codatatypes
  & & 223 & & 804 & & \bfseries \phantom{0}59 & & 1086 \\%[\jot]
Full (co)datatypes
  & & \bfseries 229 & & \bfseries 815 & & \bfseries \phantom{0}59 & & \bfseries 1103% \\[\jot]
...
increase of \OK{5.2\%}. Moreover, every aspect of the procedure, including the
...
three correspond to one-line proofs in Isabelle, of the form
\keyw{by}~\textit{coinduction}~\textit{auto}\begin{rep} \cite{blanchette-et-al-2014-impl}\end{rep}.
...
the implementation in CVC4 rely heavily on $\mu$-terms to represent cyclic values,
