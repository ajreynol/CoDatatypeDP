...
$\interp{\MU x.\: \const{C}(\MU y.\: \const{D}(x))}{\MU y.\: \const{D}(x)} =
...
$\J( \tau )$ denotes the interpretation type $\tau$ in $\J$---that is, a set of
domain elements for that type.
%For a term $t\typ{\tau}$, $\J( t ) \in \J( \tau )$ denotes the interpretation of $t$ in $\J$.
$\J( f )$ denotes the interpretation of a function $f$ in $\J$.
If $f$ is of type $ \tau_1 \times \cdots \times \tau_n \to \tau$,
then $\J( f )$ is a total function of type $ \J( \tau_1 ) \times \cdots \times \J( \tau_n ) \to \J( \tau )$.
...
\item
For each type $\tau$,
$\J( \tau )$ contains a maximal set of closed normal $\mu$-terms of that type that are
unique up to $\alpha$-equivalence and acyclic if $\tau \not\in \Codata$.
\item
For each constructor term $\const{C}( \vec t \,)$ of type $\tau$,
$\J( \const{C} )( \J( \vec t \,))$ is the value
...
if $\J( t )$ is $\MU x.\; \const{C}_{\negvthinspace j}( \vec u )$, 
then $\J\negvvthinspace( \const s^k_{\negvthinspace j} )\bigl( \J( t ) \bigr)$ is the value
in $\J( \tau )$ that is $\alpha$-equivalent to
$\interp{\J( t )}{u_k}$.
\end{enumerate}
\end{definition}

Not all normal interpretations are actually models of codatatypes, because
models must contain all possible infinite terms, not only cyclic ones. However,
only cyclic terms are interesting to us, and by monotonicity of ground
constraints it is trivial to extend a normal interpretation to an actual model.
Hence, we focus on normal interpretations.
...
$v \in \J( \tau )$
such that $v \vsimv{x} \interp{t}{u}$ for some subterm $u$ of a term $t$
...
On the other hand, if the value assigned to ${\Varec{z}}$ is fresh, %not $\alpha$-equivalent to any term in $\Terms^{\smash{\J}}_{{\Varec{z}}}( \Val )$,
...
For the latter, let $\J( x )$ be the value in $\J$ that is $\alpha$-equivalent with $\ValC \ec{x}$ for each variable $x \in \Terms(\Fc)$,
...
that $\J( t ) \aequiv \ValC \ec{t}$ for all terms $t \in \Terms( \Fc )$,
which since $\J$ is normal, implies $\J \models t_1 \teq t_2$.

If $t$ is a variable, then $\J( t ) \vsim \ValC \ec{ t }$ by construction.

If $t$ is a constructor term of the form $\const{C}( u_1, \ldots, u_n )$,
then $\J( t )$ is $\alpha$-equivalent with $\nf{\MU x.\; \const{C}( \J( u_1 ), \ldots, \J( u_n ) ) }$ for some fresh $x$,
...
Thus, $\J( t ) \vsim t_u \vsim \ValC \ec{t}$, and
we have $\J( t ) \vsim \ValC \ec{t}$.
...
If $j = \jPrime$, then $\J( t )$ is $\alpha$-equivalent with $\interp{\ValC \ec{u}}{w_k}$, which is equal to $\ValC \ec{ \smash{ \const s^k_{\negvthinspace j}( u )}}$,
which is $\ValC \ec{t}$.
If $j \neq \jPrime$, since \rn{Cong} does not apply,
any term of the form $\smash{\const s^k_{\negvthinspace j}( u' )}$ not occurring in $\ec{t}$ 
is such that $u$ and $u'$ are in distinct equivalence classes.
Due to the induction hypothesis and Lemma~\ref{lem:interpretation-completion}\PART{3}, $\J( u ) \neq \J( u' )$ for all such $u$, $u'$.
Thus, we may interpret $\smash{\J(\const s^k_{\negvthinspace j})( \J( u ) )}$ as the value in $\J( \tau )$ that is $\alpha$-equivalent with $\ValC \ec{ t }$.
...
Since $\J( t_1 ) \vsim \ValC \ec{t_1}$ and $\J( t_2 ) \vsim \ValC \ec{t_2}$,
by Lemma~\ref{lem:interpretation-completion}\PART{3}, $\J( t_1 ) \neq \J( t_2 )$, and thus $\J \models t_1 \tneq t_2$.
...
%
Discriminators are part of the signature instead
of being abbreviations.
We have found that this leads to better performance since it introduces terms less eagerly to $\tEc$.
