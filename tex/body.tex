...
$\M( f )$ denotes the interpretation of a function $f$ in $\M$.
If $f$ is of type $ \tau_1 \times \cdots \times \tau_n \to \tau$,
then $\M( f )$ is a total function of type $ \M( \tau_1 ) \times \cdots \times \M( \tau_n ) \to \M( \tau )$.
...
% AJR : rewording above "contains" makes this paragraph not necessary?  this reworded definition is more accurate and doesn't break the proofs.
%  maybe comment that acyclic infinite values are included but are irrelevant for our purposes?

%Strictly speaking, normal interpretations are not models of codatatypes,
%because they are restricted to an $\omega$-regular values, whereas actual models must
%also cater for acyclic infinite terms. However, by monotonicity of ground
%constraints, it is trivial to extend a normal interpretation to an actual model.
%In the following, we focus on the former and unapologetically call them
%models.

When constructing a model $\M$ of $\Ec$,
...
We consider a normal interpretation $\M$
