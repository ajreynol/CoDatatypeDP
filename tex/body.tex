...
(Section~\ref{sec:the-ground-decision-procedure}).
...
\label{sec:sec:the-ground-decision-procedure}
...
disequalities between $\Sig$-terms. %, taken as a conjunction.
We let $\tEc$ denote the set of $\Sig$-terms occurring in $\Ec$.
...
phase
makes \hfill inferences \hfill based \hfill on \hfill acyclicity \hfill (for
\hfill datatypes) \hfill and \hfill uniqueness \hfill (for \hfill codatatypes).

%%% TYPESETTING: horrible hack
\pagebreak[2]
\noindent
...
\rem{TODO: Normal interpretation vs. actual interpretation.}
...
$\mathcal{T}^{\smash{\J}}_{\Varec{x}}(\Val)$?)}
...
Modulo $\alpha$-equivalence, the set \smash{$\mathcal{T}^{\smash{\J}}_{{\Varec{x}}}( \Val )$} is
\[%\begin{array}{c}
\{
\MU x.\; \const{D}( x ),\;
...
$\muvar(w_i) = {\Varec{u_i}}$.  Thus by Lemma~\ref{lem:interpretation-completion}\PART{2},
...
$\muvar(w_i) = \smash{\Varec{\vphantom{I}\smash{\const s^i_{\negvthinspace\jPrime}( u )}}}$ and thus by Lemma~\ref{lem:interpretation-completion}\PART{2},
...
Due to the induction hypothesis and Lemma~\ref{lem:interpretation-completion}\PART{3}, $\M( u ) \neq \M( u' )$ for all such $u$, $u'$.
...
by Lemma~\ref{lem:interpretation-completion}\PART{3}, $\M( t_1 ) \neq \M( t_2 )$, and thus $\M \models t_1 \tneq t_2$.
