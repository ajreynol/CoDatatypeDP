...
%not occurring in $\tEc$,
and let $\Val\ec{u} = {\Varec{u}}$. Therefore, there are initially no
...
Their arguments have the same expansion with respect to
$t$:
...
construct its \emph{normal form} $\nf{t}$
...
and by recursively normalizing $t$'s subterms.
...
model $\M$ can be trivially constructed for $\Ec$. Part~\PART{1} states that the
...
required. Part~\PART{2} states that the interpretation of all subterms in the
...
Part~\PART{3} states that $\ValC$ is injective, which in the following ensures
that a distinct values can be associated with each equivalence class in a
corresponding model.
