...
\newcommand\betweenantes{\kern1.5em}
...
signature~$\Sig$. It is formulated as a calculus, whose derivation rules are
applied until saturation or contradiction.
%
The rules operate on a finite set $\Ec$ of equalities and disequalities
between $\Sig$-terms, where $\Ec$ is taken as a conjunction.
...
denoting nondeterministic branching. The rules belonging to a phase have
priority over those of the subsequent phases. The rules are applied until $\bot$
is derived or until no rule can be applied to extend~$\Ec$ further.
...
For uniqueness, if $\ec{t}$ and $\ec{u}$ are two codatatype equivalence classes
whose values $\Val \ec{t}$ and $\Val \ec{u}$ are $\alpha$-equivalent,
%for a capture-avoiding renaming of $\mu$-bound variables,
then $t \teq u$. This rule may seem overly restrictive, since
$x \teq \const{S}(x)$ and $y \teq \const{S}(\const{S}(y))$ specify the same
value despite having $\alpha$-disequivalent right-hand sides, but the
construction also works in such cases, by first deriving the equality
$y \teq \const{S}(y)$.
