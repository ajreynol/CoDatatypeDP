...
We remark that the proof of solution soundness is constructive,
meaning that it provides a method for constructing a model for a saturated configuration, by means of the mapping $\ValC$.
...
%An evaluation of the efficiency of the procedure is described by Barrett et
...
For each goal in each theory, Sledgehammer selected
\begin{conf}16~lemmas\end{conf}%
\begin{rep}a fixed number of lemmas\end{rep},
which were monomorphized and translated to SMT-LIB~2 along with the goal
\cite{blanchette-et-al-2013-smt}. The resulting problem was given to the
...
Full (co)datatypes
  & \bfseries 229 & -- & \bfseries 815 & -- & \bfseries 59 & -- & \bfseries 1103 & -- %\\[\jot]
\end{tabular}\end{center}
\caption{\,Number of solved goals with 256~lemmas per goal}
\label{tab:benchtwofiftysix}
\end{table*}
\end{rep}

The results are summarized in
\begin{conf}Table~\ref{tab:benchsixteen}\end{conf}%
\begin{rep}Tables \ref{tab:benchsixteen} and \ref{tab:benchtwofiftysix}\end{rep}.
\begin{rep}%
...
subsume the weaker ones in practice.
In particular, we encountered only one benchmark over the entire set of 4170 benchmarks
that was solved by a configuration of CVC4 and unsolved by a configuration of CVC4 with more features enabled.
...
project \relax{Hardening the Hammer} (grant NI\,491\slash 14-1).
