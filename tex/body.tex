...
After applying the substitution for the third term mentioned above\rem{which third term?},
...
framework, each $T_i$-solver for $i = 1, \ldots, l$ takes as input a combination
...
optimization for collapsing selectors) and the \rn{Single} rule are performed
eagerly---that is, for partial satisfying assignments $M$---while the rules in
phase~2 and the \rn{Split} rule are performed only for complete satisfying
assignments $M$, and in that order.\rem{in what order?}
...
Our implementation uses several optimizations not reflected in the presentation
of the calculus, which closely follow the approach outlined by Barrett et al.\
\cite{barrett-et-al-2007}.%
\rem{What closely follows the approach by Barrett et al.? The optimizations or
the calculus?}
