...
\begin{rep}\paragraph{Related Work.} Although\end{rep}%
\begin{conf}Although\end{conf} it was written a few years ago,
...
Z3 \cite{de-moura-bjoerner-2008} in unpublished work \begin{rep}by Leonardo de Moura \end{rep}and to a SPASS-like %prototypical
superposition prover \begin{rep}by Daniel Wand\end{rep} \cite{wand-2014}.
Closely related is the automatic structural
induction in superposition provers \cite{kersani-peltier-2013,wand-weidenbach-201x}
and SMT solvers \cite{reynolds-kuncak-2015}.
%
...
is associated a list of argument types $\tau_1,\ldots,\tau_n$ ($n \ge 0$)
and a return type $\tau$. This connection is written %compactly
...
then $t \teq u$. This may seem too restrictive, since
...
variables of $\Val$, namely ${\Varec{w_3}}$ and ${\Varec{x}}$.
...
Assigning a value for ${\Varec{x}}$ that is $\alpha$-equivalent to a value in $\mathcal{T}^{\smash{\M}}_{{\Varec{x}}}( \Val )$
may cause values in the range of $\Val$ to become $\alpha$-equivalent,
which in turn may cause $\M$ to
become a non-model.
% "be inconsistent." Models can't be "inconsistent".
...
After applying the substitution for $\MU {\Varec{x}}.\; \const{D}( \const{C}( {\Varec{x}} ) )$, we have that
...
model $\M$ can be trivially constructed for $\Ec$. Property~\PART{1} states that the
values in the range of $\ValC$ are $\alpha$-equivalent to a value in normal
model. In particular, this means they are closed, normal, and acyclic when
...
be associated with each equivalence class in a corresponding model.
...
framework (its variant used in CVC4 is described in~\cite{jovanovic2011sharing}), 
...
$\neg\; C$ is added to $F$, adds a clause $C$ to $F$, or does nothing.
When $M$ is a complete assignment for $F$, a theory solver can only choose to do
nothing when $M_i$ is indeed $T_i$-satisfiable.

Assume $\Ec$ is initially the set $M_i$ described above. For each equality added
...
internal state of the theory solver. The rules in phase~1 along with the
optimization for collapsing selectors are performed
...
while partitioning $S$ into $S_1, \ldots, S_n$,
where all terms in $S_i$ have the same constructor type for all their subterms of type $\Data \cup \Codata$,
and are equivalent for all subterms of type $\Nondata$.
If after doing so, some $S_i$ contains two terms $u$ and $v$, then it deduces that \rn{Unique} applies to $u$ and $v$.
...
Two paras. One general one that talks about:
\begin{verbatim}
...
\end{verbatim}
One more specific that talks about the cardinality calculus.
...
constructor $\const{C}_{\negvthinspace j}( \vec u )$, we immediately infer
$\const s^k_{\negvthinspace j}( t ) \teq u_k$, whereas the calculus we presented
would apply \rn{Split} and \rn{Inject} before inferring this equality. For the
sake of reducing the number of unique constraints considered by the calculus, we
...
CVC4, running for up to 60 seconds on the StarExec cluster
...
\begin{center}\begin{tabular}{@{\;}l@{\kern1.5em}cc@{\kern.5em}cc@{\kern.5em}cc@{\kern1.3em}cc@{\;}}
  & \multicolumn{2}{c}{Distro} & \multicolumn{2}{c}{AFP} & \multicolumn{2}{c}{\gandl} & \multicolumn{2}{c}{All} \\
  & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4} & \HD{Z3} & \HD{CVC4}
\MIDRULE
No (co)datatypes
  & 209 & 221 & 777 & 785 & \phantom{0}51 & \phantom{0}52 & 1037 & 1048 \\[\jot]
%Datatype simplication only
%  &  \\
Datatypes w/o \rn{Acyclic}
  & & 235 & & 770 & & \phantom{0}61 & & 1066 \\
Full datatypes
  & 213 & 227 & 791 & 786 & \phantom{0}51 & \phantom{0}52 & 1055 & 1065 \\[\jot]
%Codatatype simplication only
%  &  \\
Codatatypes w/o \rn{Unique}
  & & 230 & & 794 & & \phantom{0}65 & & 1089 \\
Full codatatypes
  & & 231 & & 794 & & \bfseries \phantom{0}68 & & 1093 \\[\jot]
Full (co)datatypes
  & & \bfseries 237 & & \bfseries 802 & & \bfseries \phantom{0}68 & & \bfseries 1107% \\[\jot]
...
increase of \OK{4.9\%}. Moreover, every aspect of the procedure, including the
...
\keyw{by}~\textit{coinduction}~\textit{auto} \cite{blanchette-et-al-2014-impl}.
...
%% alliteration
On the practical side, we obtained respectable results on Isabelle benchmarks. \rem{emphasize new capabilities allows us to improve state of the art.}
...
proof reconstruction for CVC4's (co)datatype inferences in Isabelle. We also
want to try out CVC4 for higher-order model finding in Isabelle, as an
alternative to the counterexample generator Nitpick
\cite{blanchette-nipkow-2010}. Finally, it might be worthwhile to enrich SMT
solvers with explicit support for recursive and corecursive functions.
...
who jointly with the first author developed the theory solver for datatypes in
CVC4.
%
