...
\newcommand\Nondata{\Types_{\mathrm{un}}}
...
non-redun-dant counter-examples counter-example}
...
(model soundness) on universal problems. %and the absence of spurious models.
...
\begin{rep}\ourparagraph{Related Work.} Barrett\end{rep}%
...
\emph{uninterpreted types}. The function symbols are partitioned into $\Funcs = \Ctr
...
uninterpreted, among the already known (co)datatypes, or among the (co)datatypes
being introduced.
%
...
\end{rep}%
This schema encodes a form of extensionality:\ Two values that yield the same
...
Dually, coinduction can be replaced by the uniqueness axiom schema, which
asserts that codatatype values are fully characterized by their possibly
infinite expansion \cite[Theorem 8.1, 2$\,{\Leftrightarrow}\,$5]{rutten-2000}.
...
Fortunately, \begin{rep}given cardinalities for the uninterpreted types, \end{rep}%
...
and disequalities between $\Sig$-terms. The
...
datatypes.
Second, all uninterpreted types have infinite cardinality.
Without quantifiers, the constraints $\Ec$ cannot entail an upper
bound on the cardinality of any such type, so it is safe to consider these types
infinite.%
%\footnote{%
%The theory could be extended This is not very restrictive, because in principle each interpreted type in
...
implementation within the SMT solver CVC4:\ the integration into CDCL($T$), the
extension to quantified formulas, and some of the optimizations.
...
and the equivalence of their arguments of uninterpreted types.
...
that define codatatypes falling within the supported fragment. \hfill We~also%
\begin{conf}%
\par
%%% TYPESETTING: Terrible hack
\pagebreak
\noindent
\end{conf}%
...
success rate increase of over \OK{5\%}, which is quite significant. For Isabelle
...
(DFG) project \relax{Hardening the Hammer} (grant Ni\,491\slash 14-1).
