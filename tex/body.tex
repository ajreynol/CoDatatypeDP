...
\newcommand{\tEc}{\mathcalx{T}(\Ec)}
\newcommand{\tcEc}{\mathcalx{T}^\AST(\Ec)}
...
The mapping $\Val$, on which the rules depends, is computed as follows.
%
...
$\mathcalx{T}(\Fc) \subseteq \tcEc$, and hence contains an equality between two terms from $\tcEc$ not occurring in its parent node.
...
we write $\mathcalx{T}^{\smash{\J}}_x( \Val )$ to denote the set consisting of all values
$v \in \J( \tau )$
...
\inferrule{
  ...
  v \in \J( \tau )
  \betweenantes
  v \not\in \mathcalx{T}^{\smash{\J}}_{\Varec{x}}(\Val)
}{
...
value---one that does not occur in $\mathcalx{T}^{\smash{\J}}_{\Varec{x}}(\Val)$
...
%$\mathcalx{T}^{\smash{\J}}_{\Varec{x}}(\Val)$?)}
...
the range of $\Val$ after each rule application.%\rem{TODO: clumsy}
...
variables of $\Val$, namely ${\Varec{z}}$ and ${\Varec{y}}$.
Modulo $\alpha$-equivalence, the set \smash{$\mathcal{T}^{\smash{\J}}_{{\Varec{z}}}( \Val )$} is
$%\begin{array}{c}
\{
\MU z.\; \const{D}( z ),\;
\MU z.\; \const{C}( \const{D}( z )),\allowbreak\;
\const{C}( \MU z.\; \const{D}( z )),\;
\MU z.\; \const{C}( z )
\}
%\end{array}
$.
...
Assigning a value for $\Val \ec{z}$ that is $\alpha$-equivalent to a value in $\mathcalx{T}^{\smash{\J}}_{{\Varec{z}}}( \Val )$
...
After this substitution, $\Val \ec{v}$ is $\MU {\Varec{v}}.\; \const{D}( \MU {\Varec{z}}.\; \const{D}( {\Varec{z}} ) )$,
...
On the other hand, if the value assigned to ${\Varec{z}}$ is fresh, %not $\alpha$-equivalent to any term in $\mathcalx{T}^{\smash{\M}}_{{\Varec{z}}}( \Val )$,
...
For example, after assigning $\MU {\Varec{z}}.\; \const{D}( \const{C}( {\Varec{z}} ) )$ to $\Varec{z}$, we have that
$\Val \ec{u}$ is $\MU {\Varec{u}}.\; \const{C}( \MU {\Varec{z}}.\; \const{D}( \const{C}( {\Varec{z}} ) ) )$,
...
and thus there are no terms in $\mathcalx{T}^{\smash{\J}}_{\Varec{y}}(\ValC)$ of type $\tau$.
...
since there are only a finite number of closed terms in $\mathcalx{T}^{\smash{\J}}_{\Varec{y}}(\ValC)$,
and thus %our %model
completion can assign a value not occurring in $\mathcalx{T}^{\smash{\J}}_{\Varec{y}}(\ValC)$ to ${\Varec{y}}$.
...
${\Varec{x}} \in FV( \Val_1 )$,
and $\MU {\Varec{x}}.\; t$ is not $\alpha$-equivalent to a term in $\mathcalx{T}^{\smash{\J}}_{\Varec{x}}(\Val_1)$.
To show 2,
...
there is a closed $\mu$-term $v \in \mathcalx{T}^{\smash{\J}}_{\Varec{x}}( \Val_1 )$
...
For the latter, let $\M( x )$ be the value in $\M$ that is $\alpha$-equivalent with $\ValC \ec{x}$ for each variable $x \in \mathcalx{T}(\Fc)$,
...
that $\M( t ) \aequiv \ValC \ec{t}$ for all terms $t \in \mathcalx{T}( \Fc )$,
...
When assuming that a finite type has infinite cardinality, the calculus presented in this paper is \relax{incomplete},
due to the overly restrictive preconditions on \rn{Split} and \rn{Single}.
...
The implementation of the decision procedure uses several optimizations which
