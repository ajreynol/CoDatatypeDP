...
Ensuring disequality constraints are satisfied by an interpretation based on $\ValC$
relies on showing that $\ValC$ is injective modulo $\alpha$-equivalence.
The last precondition in the above rule ensures that $\ValC$ remains injective after each substitution.
More precisely, the set $\Terms_{\smash{\J}}^{\Varec{x}}(\ValC)$ is an overapproximation of the values
that, when assigned to ${\Varec{x}}$, cause values in the range of $\ValC$ to become $\alpha$-equivalent.
...
due to the fact that $\Terms_{\smash{\J}}^{\Varec{x}}(\ValC)$ is a finite set.
%Injectivity is important to satisfy disequality constraints.
%For infinite codatatypes, it is always possible to find suitable values $v$.
%For example,
...
empirically advantageous to delay applications of \rn{Split} as long as
